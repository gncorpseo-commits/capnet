...
> **갱신: 2026-09-06** · 종착점 = **Phase 3+ 전체** (D16) · 제품 유통 = **D19** · 출품 후 = **D25 트랙 A** · README는 상태 비보유(링크만)
...
> | 실제 결함으로 고친 것 | **6** (하네스 부재 · 주석 우회 15곳 · 세대 표 정지 · SBOM 도구 무버전 · 자기 검사 정정 · 경계값 검사 부재) |
> | 0건 핀 13 · 0+재현/표 8 | Decision 구현 0 · DDL 0 · 정책 숫자 0 |
> | **태그 zip** | `check_release.sh v0.1.0-contest` → **rc 1** (태그 뒤 늘어난 가중치 셋) — 태그 이동 없음 · Decision 감 (inbox #159 표) |
> | master 머지 대기 | `#311` · `#315`(이 Step 0) |
>
> 새 도구: `scripts/check_test_order.py` · `scripts/mutation_harness.py`. Docker 없어 못 본 것은 inbox #159 표 3번.
>
> **다음:** **최종 G 한 바퀴 끝** (G1 `#339` · G2 `#338` · G3–G5 0+재현) · 시드 종료 · **Decision만 남음** — inbox `#159` 표가 사람 몫. 검사 **1235**. 새 시드 번호 없음. 세션 종료 조건 1 충족.

> **13회차 (2026-09-07) — 배치 C(101–130) 소진 · PR 열여덟 (`#297`–`#314`) · 즉시 D.**
>
> | 무엇 | 값 |
> |---|---|
> | 검사 | 1135 → **1187** · 건너뜀 7 · 실패 0 — 재현 `bash scripts/run_tests.sh` |
> | 실제 결함으로 고친 것 | **5** (규칙 3 우회 셋 · 규칙 2 우회 둘 · zip 금지 목록 · 예외 문구의 셀 값 · 재현 절 없던 검사) |
> | 0건 핀 11 · 0+재현 5 · 표만 7 | Decision 구현 0 · 정책 숫자 0 |
> | master 머지 대기 | `#311`(Node 런타임 두 줄) + 이 Step 0 |
>
> 직렬 PR 로 CHANGELOG 충돌 0. Docker 없어 못 본 것: 두 워커 이중 claim · RUNNING 행 실측 · 골든 누출 검사 실행.
>
> **다음:** G1–G5 → 배치 D(131–160) — Cursor 재전달 없이 이어간다 (Decision `through-final-continuous-c-d`).
...
> | 실제 결함으로 고친 것 | **11** (키 아홉 번 · 주석 우회 셋 · 세는 도구 둘 · GC 회수량 · 버전 폴백 침묵 …) |
> | 0건으로 못박은 것 | **13** |
> | master 머지 대기 (경계 밖) | **6** — `#277` `#280` `#285` `#287` `#293` + Step 0 |
>
> **머지 권한 (사용자 승인 · `#265`)** — `tests/`·`docs/`·`scripts/` 안 · CI 3/3 · 뮤테이션 ≥2 일 때만 내가 머지했다.
> `#290` 은 체크 둘이 도는 중에 머지했다 — 위반이고 inbox 에 적었다. 이후 버킷 `pass` 게이트.
>
...
> | G | PR | 무엇 |
> |---|---|---|
> | G1 | #258 | **설정을 주석으로 옮겨도 통과**하던 검사 둘 (내가 쓴 것) |
> | G2 | #259 | 데모가 등록하는 `arch` 이름은 아무도 안 봤다 |
> | G3 | #260 | 운영 지표 넷 · 대회 「외부 AI API 0건」에 검사 없음 |
> | G4 | #256 | CHANGELOG 선두가 가리키는 곳이 있는가 (큐 #69) |
> | G5 | #261 | 로컬도 CI 도 **안 보는 검사 한 건** — 차이를 1 로 못박음 |
>
> **G1 이 가장 크다.** `#252`·`#251` 의 검사가 설정을 `#` 로 주석 처리해도 통과했다 —
> migrate 무한 재시작 · Node 가 증서 없이 도는 모양이 안 걸렸다.
> `_srcguard.hash_comment_free()` 로 막았다.
>
> 검사 **949 → 973** (건너뜀 7 · 실패 0).
>
> **다음:** 배치 B(71–100) **미기입**. §4 가 채워질 때까지 번호를 발명하지 않는다.

> **11회차 마감 (2026-09-05) — 배치 A **30줄 전부** 소진 · PR **서른하나** (`#226`–`#257`).**
>
> 시드 41–70 을 우선순위대로 비웠다.
>
> | 무엇 | 값 |
> |---|---|
> | 검사 | **707 → 941** · 건너뜀 7 · 실패 0 |
> | Core SQL 참조 | 335 → **449** (뷰 컬럼 27 · f-string SQL 87 이 처음 검사 안으로) |
> | 실제 결함으로 고친 것 | **8** |
> | 「오늘 0건」을 못박은 것 | 12 |
>
> **고친 결함 여덟**
...
> | PR | 무엇 |
> |---|---|
> | #226 | 새 런북이 `gh … list` 검사 밖 (**main 빨강**) |
> | #237 | `ccurl` 이 **API 키를 `ps` 에 노출** — `-H @파일` 로 |
> | #238 | 제출 정본 체크리스트의 `prod_room 27/27` 이 낡음 (실제 51) |
> | #239 | `generate_sbom` 이 의존성 없이 조용히 SBOM 을 만듦 |
> | #240 | bash 로 문서대로 돌리면 A/B 비교가 **자기 산출물을 못 찾음** |
> | #244 | 실패한 회차에는 **건너뜀 수가 안 보임** |
> | #252 | migrate 대기가 **조용히 만료** |
> | #255 | capreq 로컬 실행이 `FAILED (errors=3)` (규약은 건너뜀) |
>
> **환경:** Docker **데몬 없음**. compose **CLI 는 있어**(v5.3.1) `config` 병합은 쟀다.
> 못 잰 것 — `prod_room` 에 `-e` 켜고 51/51 재측(#44) · 능력 종단 데모 아홉을
> clean_room 에 넣기(#46) · `regate`·`proof_ab` 본실행(#63) · capreq **72**(#60).
>
> **큐 #70:** 열린 Decision **20** 중 **여섯은 코드가 이미 나갔다** (`capability-catalog-52` ·
> `org-boundary` · `read-auth-ownership` · `safety-chain` · `step5-text-executor` ·
> `step6-prep`). **status 는 안 내렸다** — 남의 판정을 내 손으로 닫지 않는다.
>
> **다음:** 배치 B(71–100)는 **미기입**. 채워지기 전까지는 G1–G5.

> **11회차 진행 (2026-09-05) — 배치 A 착수 전에 `main` 이 빨갰다.**
> `#225` 가 네 번째 런북 `queue-batches.md` 를 만들자 `#220` 의 `gh … list` 검사가
> **손으로 적은 런북 셋**만 보고 새 파일을 놓쳤다 — 세는 자리 3 → **2**, 바닥이 울었다.
> 목록을 버리고 `docs/bridge/*.md` 훑기 + 우편함 둘 제외로 바꿨다 (뮤테이션 3).
> 709 통과 · 건너뜀 7. 이어서 큐 **#41**.
...
| **배정 재시도 상한 (2026-08-13)** | ✅ **조용한 무한 재시도 종결.** `claim` 이 `attempt_no` 계수 · `capability.max_attempts`(기본 5) 도달 시 워커가 task `FAILED` · Node 가 `/fail` 로 보고해 `audit_log` 에 이유 기록 · DB 가 상한 초과 배정 거절. **실측: 3회에서 정지, 20초 뒤에도 추가 배정 0** · 골든 `acc=0.8500` 불변 (`demo.sh`) |
