...
> **11회차 (2026-09-05) — 배치 A 우선순위 첫 줄 소진 · PR 여덟 (`#226`–`#233`).**
>
> `main` 이 **빨간 채로** 시작했다 — `#225` 가 만든 네 번째 런북 `queue-batches.md` 가
> `#220` 의 검사 밖이었다 (세는 자리 3 → 2).
>
> | PR | 큐 | 무엇 |
> |---|---|---|
> | #226 | — | 새 런북이 검사 밖 (**main 빨강 복구**) |
> | #227 | 41 | 뷰 컬럼 사각 **27건** — 참조 335 → **362** |
> | #228 | 44 | `set -e` 없는 스크립트 **1** · 세던 검사 0 |
> | #229 | 46 | 「전부 재현」이 능력 **10 중 1** 이었다 |
> | #230 | 50 | 바닥을 내리면 초록 — **92건** 등록부 |
> | #231 | 43 | 손 예외 목록 **12** · 늘어도 조용했다 |
> | #232 | 42 | 통합 검사 **15** 는 CI 한 줄로만 돈다 |
> | #233 | 45 | `!override` 는 덮는다 · 문서가 요구 버전을 낮게 적었다 |
>
> 검사 **707 → 763** · 건너뜀 7 · 실패 0. 한 갈래 스택이라 꼭대기만 squash.
>
> **환경:** Docker **데몬 없음** (10회차와 다르다). compose **CLI 는 있어**(v5.3.1)
> `config` 병합은 쟀다. 못 잰 둘 — `prod_room` 에 `-e` 켜고 51/51 재측(#44) ·
> 능력 종단 데모 아홉을 clean_room 에 넣기(#46). Docker 회차의 첫 줄.
>
> **다음:** 49 → 64 → 47 → 48.
