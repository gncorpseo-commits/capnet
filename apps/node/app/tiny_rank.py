"""규칙 기반 순위 — 가중치 파일은 증적용 자리표시자뿐이다.

추론은 `rank_rules` 의 토큰 겹침으로 한다. `RuleTextRank` 는 safetensors·지문·
`max_params` 게이트가 성립하도록 **파라미터 0 인 torch 모듈**이다 — 「모델 없이도 됨」(step6 §3).
`RuleTextNer`·`RuleTextExtract` 와 같은 모양이고, 같은 이유로 버퍼 한 칸을 둔다.
"""
