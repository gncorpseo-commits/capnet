...
1. Issue 먼저 (`W1:` …).
2. 브랜치 `finn/<topic>` 또는 `toma/<topic>` (`main` 직접 push 금지).
3. 커밋 author는 `finn` / `toma` (전역 git config 금지 — `-c user.name=...` 권장).
4. PR → 상대 **명목 승인** 코멘트 `LGTM (finn|toma)`.
5. **master**가 Squash merge.
