...
4. **같은 조직** `developer` 는 남의 작업도 본다 (운영) — 조직 없는 `developer` 는 **못 본다**.
   조직 없는 `admin` 은 팀 운영자로 보고 전체를 본다 (D24)
...
def main() -> int:
    ...
    with psycopg.connect(settings.database_url, row_factory=dict_row, autocommit=False) as conn:
        ...
        # 조직 없는 developer 와 조직 없는 admin(팀 운영자)을 구별해서 본다 (D24)
        stray = ensure_user(conn, name="stray-probe", role="developer")
        team = ensure_user(conn, name="team-probe", role="admin")
        org_id = conn.execute(
            "INSERT INTO org (code, name) VALUES ('own-probe','own-probe') RETURNING id"
        ).fetchone()["id"]
        for u in (owner, other, oper):
            conn.execute("UPDATE app_user SET org_id=%s WHERE id=%s", (org_id, u["id"]))
        ...
        stray_key = issue_key(conn, user_id=stray["id"], label="stray")["secret"]
        team_key = issue_key(conn, user_id=team["id"], label="team")["secret"]

        task_id = conn.execute(
            """
            INSERT INTO task (user_id, capability_id, status, trust_domain,
                              capability_trust_domain_min, input_ref, result_ref, org_id)
            SELECT %s, c.id, 'COMPLETED', 'team', c.trust_domain_min,
                   '{"caseId":"ic1-0001"}', '{"label":"annual_crop"}', %s
              FROM capability c WHERE c.id = %s
            RETURNING id
            """,
            (str(owner["id"]), str(org_id), CAP),
        ).fetchone()["id"]
        ...
        try:
            ...
            # D24 이후 운영자 규칙은 **조직에 달려 있다.**
            got = m.get_task(task_id, f"CapNet-Key {oper_key}")
            check(str(got["id"]) == str(task_id),
                  "같은 조직 developer 는 남의 작업도 본다 (운영)")
            check(status_of(m.get_task, task_id, f"CapNet-Key {stray_key}") == 404,
                  "조직 없는 developer 는 못 본다 (D24 로 좁아진 지점)")
            got = m.get_task(task_id, f"CapNet-Key {team_key}")
            check(str(got["id"]) == str(task_id),
                  "조직 없는 admin 은 팀 운영자 — 전체를 본다")
            ...
            ids = [str(u["id"]) for u in (owner, other, oper, stray, team)]
            conn.execute(
                "DELETE FROM api_key WHERE user_id = ANY(%s)",
                (ids,),
            )
            conn.execute("DELETE FROM app_user WHERE id = ANY(%s)", (ids,))
            conn.execute("DELETE FROM org WHERE id = %s", (str(org_id),))
            conn.commit()
            left = conn.execute(
