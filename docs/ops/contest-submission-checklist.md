...
| S4-1 | **반드시 넣는 것**: 데모용 가중치 **5종** + `placeholder` | 없으면 심사위원이 재현할 수 없다. `check_submission` 의 `REQUIRED_WEIGHTS` 가 정본 · `check_release.sh` 가 압축본에서 확인한다 |
...
- [ ] **자체 scratch 가중치 5종 + `placeholder` 는 저장소에 유지** — 삭제 금지.
