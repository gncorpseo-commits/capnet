...
이 두 줄을 **그대로** 쓴다. 순서도 바꾸지 않는다.

> **A-1.** 같은 능력으로 **다른 에이전트에 교체 배정**됩니다. 계약을 통과한 것만 후보가 됩니다.
> **A-2.** 다만 **두 에이전트가 같은 답을 낸다고는 말하지 않습니다.**

**A-2 를 빼지 않는다.** 이 구간에서 가장 중요한 문장이고, 빼면 A-1 이 등가 주장으로 읽힌다.
편집에서 시간이 모자라면 **A/B 구간 전체를 들어내지, A-2 만 지우지 않는다.**

#### 쓰면 안 되는 것 (전부 금지)

| 금지 문구 | 왜 |
|---|---|
| 「**Within**」 · 「편차 0.05 이내」 | 하한형 게이트는 통과자 쌍의 편차를 유계로 만들지 못한다 (SD-009) |
| 「**\|Δacc\|≈0.047**」 · 「n300 `\|diff\|≤0.05`」 | **누출된 골든셋**으로 잰 값이다 — 골든셋이 학습셋 안에 있었다 (roadmap §1.2) |
| 「같은 답」 · 「동일한 결과」 · 「대체 가능」 | D17 이후 **등가성은 계약 보장이 아니라 관측값**이다 |

홀드아웃 n=300 재측정은 **0.0967 · EXCEEDS** 다. 옛 문구가 화면에 나가면
**반증된 보장을 출품 영상에서 주장**하게 된다.

#### 화면에 나와도 되는 사실

`proof_ab.sh` 가 보여 주는 것은 **「교체 배정이 사슬 위에서 둘 다 완결된다」**뿐이다
(2026-08-08 실측 · `honored=true` · assignment 2건 SUCCEEDED). 그 이상은 말하지 않는다.
**점수 숫자를 화면에 띄우지 않는다** — 띄우면 자막이 무엇을 말하든 시청자가 비교한다.
