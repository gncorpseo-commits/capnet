...
>
> **`clean_room` 은 2026-09-04 에 이 WSL 에서 실제로 돌았다 — 통과 9 · 실패 0 ·
> 실게이트 `acc=0.8500 f1=0.8344`.** 8·9회차는 `docker info` 가 실패해 두 번 「못 봤다」로
> 남겼던 자리다. `prod_room` 은 **아직 이 환경에서 못 돌렸다** — 27/27 은 미확인이다.
