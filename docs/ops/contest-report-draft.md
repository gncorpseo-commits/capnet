...
입력 3종·출력 2종이 **같은 계약 게이트와 배정 경로**로 돌았다. 각 능력은 계약이 선언한
전처리를 러너가 **적용해** 실행하고, 출력을 계약과 대조한 뒤 통과한다.

**다섯 중 넷은 품질을 주장하지 않는다.** 여기서 보이는 것은 모델 성능이 아니라
**계약과 라우팅이 능력 종류에 매이지 않는다**는 것이다. 품질 주장은 아래 선택
프로파일을 붙인 능력에서만 한다.

재현: `bash scripts/text_demo.sh` · `embed_demo.sh` · `series_demo.sh` ·
`image_embed_demo.sh` — 넷 다 마지막 줄에 **무엇을 주장하지 않는지**를 스스로 출력한다.
