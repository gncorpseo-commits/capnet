...
채점 가능성을 요구하지 않는다 — 그래서 분류 밖으로도 붙는다.

**실측 — 능력 6종이 같은 사슬을 통과했다**

| 능력 | 입력 | 출력 | 품질 프로파일 |
|------|------|------|---------------|
| `image.classify` | 이미지 | 닫힌 라벨 | **golden** (아래) |
| `text.classify` | 텍스트 | 닫힌 라벨 | none |
| `image.embed` | 이미지 | 벡터 128 | none |
| `text.embed` | 텍스트 | 벡터 64 | none |
| `timeseries.forecast` | 표(CSV/JSON) | 수치 배열 4 | none |
| `table.extract` | 문서(평문 표) | 여러 칸(열·행·머리글) | none |

입력 4종·출력 3종이 **같은 계약 게이트와 배정 경로**로 돌았다. 각 능력은 계약이 선언한
전처리를 러너가 **적용해** 실행하고, 출력을 계약과 대조한 뒤 통과한다.

**여섯 중 다섯은 품질을 주장하지 않는다.** 여기서 보이는 것은 모델 성능이 아니라
**계약과 라우팅이 능력 종류에 매이지 않는다**는 것이다. 품질 주장은 아래 선택
프로파일을 붙인 능력에서만 한다.

재현: `bash scripts/text_demo.sh` · `embed_demo.sh` · `series_demo.sh` ·
`image_embed_demo.sh` · `table_demo.sh` — 다섯 다 마지막 줄에 **무엇을 주장하지 않는지**를
스스로 출력한다.
...
>
> **그리고 그 「중개」를 끝까지 밀었다.** 고친 직후에도 중개되는 것은 **배정뿐**이었고 데이터는 기기가 로컬에 미리 둔 것에서 골랐다. 지금은 **입력 바이트도 Core가 받아 보낸다** — 주장과 동작의 일치를 데이터에까지 넓힌 것이 이 프로젝트의 가장 중요한 수정이다.
...
촬영 자산이 이 6종 기준이라 시연 목록은 그대로 두되, 이후 같은 방식으로 제약을 더 늘렸다 — **재시도 상한 초과 배정**(`ck_assignment_attempt_within_cap`) · **샘플 없는 계약 심사** · **초대 등급·소진 한도**(`ck_invite_domain` · `ck_invite_redeem_within_cap`). 거절의 주체는 계속 DB다.

---

## 5.5 입력 통제와 계약 검증

**입력.** 금지하는 것은 「자유 업로드」가 아니라 **비통제 수집**이다. Core가 바이트를 받아
sha256·크기·MIME를 능력 계약과 대조하고(초과·미선언은 DB가 거절), 배정과 함께 기기로 보낸다.
기기는 받은 바이트를 **다시 해시해 대조한** 뒤에만 실행하고, 끝나면 지운다.
바이트는 휘발성이고 **해시와 증적은 남는다** — 1절의 「어디로 갔는지 답할 수 있는가」에
답하는 근거가 이것이다. 서명 URL·다운로드 토큰 형태는 다운로드 이후를 통제하지 못하므로 채택하지 않았다.

**계약 검증.** 이전에는 제출자가 「계약을 지킨다」고 보고하면 그대로 적었다. 지금은 채점 기기가
**실행해서** 판정한다 — 선언한 아키텍처로 가중치가 로드되는지, 파라미터 수가 상한 안인지,
샘플 입력이 실제로 추론되는지, 출력이 스키마를 만족하는지, 선언한 전처리가 그대로 쓰이는지.
선언과 다른 아키텍처를 적은 에이전트는 그 자리에서 탈락한다.

**초청 경로.** 조직 밖 기기는 **초대장에 등급이 박힌 채** 들어온다. 신청자는 등급을 고르지
않고 소진할 뿐이며, 초대로 들어온 기기는 채점자가 될 수 없다(제약이 막는다).
