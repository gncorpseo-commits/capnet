...
**증명한 것 (초안 시점):** 스키마 불변식 실측, dummy E2E 배관, 골든셋 데모 N=40 핀, 위반 6종 스크립트.  
**아직 아닌 것:** EuroSAT scratch 실게이트 채점, `scripts/demo` 단일 명령, 시연 영상.  
**재현:** `docker compose up --build` → `scripts/smoke_w1.ps1` → `scripts/demo_violations.ps1`.
...
- 통과 기준 AND (`min_accuracy` · `min_macro_f1` · `max_invalid_rate`) — 숫자는 아직 가정값
...
- `min_accuracy` 0.75는 가정. scratch 베이스라인 실측 후 고친다.
