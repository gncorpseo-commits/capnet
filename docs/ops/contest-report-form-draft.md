...
| 상용 AI 보조도구 활용 여부 및 범위 | 코드 작성·디버깅·문서 정리 보조로 Cursor 및 Claude Code를 활용함. **[TODO: 전체 비율 확정]** 스키마 제약·게이트 사슬·채점 규칙·판정 기준은 팀이 설계했고, AI 생성 코드는 팀이 동작 원리를 확인하고 실행 검증한 뒤 반영함. |
