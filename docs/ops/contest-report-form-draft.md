...
라우팅 불변식을 앱이 아니라 **PostgreSQL이 거절**한다. 위반 14종 실측·M25 6종 스크립트. 게이트는 team runner만. 서로 다른 scratch 백본 A/B를 같은 계약으로 게이트에 통과시킨 뒤 동일 입력에 교차 배정했고, 둘 다 완결됐다. 정확도 차는 n=300에서 `|Δacc| = 0.0467`로 기준 이내였다 — 다만 **이 조건에서 반증되지 않았다**는 뜻이며, 대체 가능성이 입증된 것은 아니다 (홀드아웃·epoch·SE 한계는 한계점 절).
