...
>
> **1-1) 실측 — 능력 5종이 같은 사슬을 통과했다.** `image.classify`(이미지→라벨) · `text.classify`(텍스트→라벨) · `image.embed`·`text.embed`(→벡터) · `timeseries.forecast`(표→수치 배열). **입력 3종**(이미지·텍스트·표)과 **출력 2종**(닫힌 라벨·구조화)이 같은 계약 게이트와 배정 경로로 돌았다. 다섯 중 **넷은 품질을 주장하지 않는다**(선택 프로파일 미사용) — 여기서 보이는 것은 모델 성능이 아니라 **계약과 라우팅이 능력 종류에 매이지 않는다**는 것이다.
>
...
> ```
> 넷 다 마지막 줄에 **무엇을 주장하지 않는지**를 스스로 출력한다.
