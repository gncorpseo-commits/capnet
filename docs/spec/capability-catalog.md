...
> 하네스가 없어 **아무도 재현할 수 없었다.** 뒤에 `scripts/route_bench.py` 로 능력 9종을 모두
> 등록하고 **홀드아웃 12개 × R=5** 로 다시 재니 **40/60** 이다. 숫자를 지우지 않고 옆에 적는다 —
> **경계 문장의 방향은 맞았다**(같은 홀드아웃에서 옛 설명 30/60 → 저장소 설명 40/60).
> 틀렸던 것은 **작은 자기 선택 표본으로 「고쳤다」고 말한 것**이다.
