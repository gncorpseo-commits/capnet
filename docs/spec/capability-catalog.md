...
| 49 | `safety.pii` | text | `structured` | none | v제품-1 ✅ **구현됨** |
...
는 그대로다 — 골든 경로의 픽셀 처리가 바뀌면 `acc=0.8500`(`demo.sh`) 이 움직인다.
...
> **그 「4/5 → 5/5」가 무엇이었는지 (2026-08-30 정정).** 그때 **손으로 고른 프롬프트 5개**였고
> 하네스가 없어 **아무도 재현할 수 없었다.** 숫자를 지우지 않고 **그게 무엇이었는지**를 옆에
> 적는다 — 작은 자기 선택 표본이다. 되풀이하려면 `scripts/route_bench.py`.
>
> **한 번 더 정정 (2026-08-31).** 이 자리에 처음 적었던 「저장소 설명 **40/60**」은
> **철회한다.** 그 값은 하네스의 `--descriptions repo` 로 잰 것인데, 그때 그 경로가
> `CapabilityInfo` 를 새로 지으면서 **`output_kind` 를 떨어뜨리고 있었다.** 라우터 프롬프트는
> `kind=` 를 넣으므로, 그 조건은 「설명만 바꾼 카탈로그」가 아니라 **「설명을 바꾸고 kind 를
> 전부 지운 카탈로그」**였다 — 한 번에 둘을 바꿔 놓고 설명 덕이라 읽었다.
> 래퍼를 고친 뒤 같은 조건은 **37/60** 이다.
>
> **지금 서 있는 값** (홀드아웃 × R=5 · `qwen2.5:3b`):
>
> | 언제 | 값 |
> |---|---|
> | DB 설명이 저장소보다 낡았을 때 | **30/60** (1회 · 밴드를 모른다) |
> | 데모가 설명을 동기화한 뒤 | **36 · 36 · 38** (3회) · `repo` 조건 **37** |
>
> **같은 조건도 2점씩 흔들린다.** 그래서 이 표는 「좋아졌다」가 아니라 **「무엇을 재면 무엇이
> 나오는가」**로만 읽는다. 30 은 한 번만 잰 값이라 그 밴드를 모른다 — **여기서 개선폭을
> 말하지 않는다.**
>
> **능력 10종 · 홀드아웃 13개 (2026-09-01 · Wave N).** `safety.pii` 를 더하고 그 프롬프트를
> 세트에 넣은 뒤: **42/65.** 새 능력은 **5/5** 로 자기 것을 가져갔고, **기존 12개는 37/60** 으로
> 위 밴드(36·36·38) 안이다 — **10번째 능력이 이웃을 밀어내지 않았다.**
> 재현: `PYTHONPATH=capreq/src python3 scripts/route_bench.py --set holdout --repeats 5`
> (능력 10종 등록 선행 · Ollama 필요). **개선을 주장하지 않는다.**

**새 학습이 없다.** `RuleTextExtract` 는 파라미터 0 이고 `rule_extract.safetensors` 는
버퍼 한 칸짜리 자리표시자다 — 그래서 **`rule_ner.safetensors` 와 바이트가 같다**
(sha `15458b00…`). `text.rank` 의 `rule_rank.safetensors` · `safety.pii` 의
`rule_pii.safetensors` 까지 **넷 다 바이트가 같다**.
...
> **위 표의 한 줄은 틀렸다 (2026-08-30 정정).** 「제목·담당자 같은 항목 뽑아줘」가
> `text.ner` 로 간다(❌)고 적었지만, 그건 **능력 5종만 등록된 스택에서 n=1** 로 본 것이다.
> `scripts/route_bench.py` 로 **9종을 모두 등록하고 R=5** 로 재니 **5/5 로 `text.extract`** —
> 맞게 간다. **그 미스 보고는 취소한다.**
>
> 재현되는 미스는 따로 있다: 「이 글에 나오는 날짜랑 URL 전부 뽑아줘」 → `text.extract`
> **5/5**(`text.ner` 이어야 한다). 설명을 손봐 이것을 고쳐 봤지만 **홀드아웃에서 순 효과가
> 0** 이라(미스 하나를 고치고 다른 하나를 깼다) **적용하지 않았다.**
>
> 배운 것은 능력이 아니라 **재는 방법**이다 — 작은 자기 선택 표본으로 라우팅을 말하지 않는다.
> 되풀이하려면 `scripts/route_bench.py`.

**설명은 데모가 DB 에 맞춘다 (2026-08-31 · Wave I).** 등록(`POST /v1/capabilities`)은
`(code, version)` UNIQUE 로 한 번뿐이라, **저장소에서 설명을 고쳐도 이미 등록된 스택에는
안 들어갔다** — 데모가 충돌을 삼키고 기존 id 만 찾아 썼기 때문이다. 라우터는 DB 의 설명을
읽으므로 오래 돌아간 스택은 저장소와 다른 문구로 라우팅했다.

이제 `ner_demo.sh`·`text_extract_demo.sh`·`text_rank_demo.sh` 가 기존 능력을 만나면
`PATCH /v1/capabilities/{id}` 로 **설명만** 맞춘다. **정본은 저장소의 등록 본문**이고 DB 를
거기에 맞출 뿐이다 — **문구를 데모에서 새로 짓지 않는다.** 계약 칸은 PATCH 가 받지 않는다
(400): 그것들은 `task_input` 복합 FK · `gate_run` · `assignment` **스냅샷의 원본**이고,
원본이 움직이면 이미 찍힌 스냅샷이 거짓말이 된다. 바꾸려면 새 버전을 등록한다.
...
### `safety.pii` — **탐지가 아니라 참고** (Wave L · 2026-08-31)

**이름이 위험한 능력이다.** 「PII 를 찾는다」가 놓치면 없느니만 못하다 — 사람은 결과가 비면
「검사했으니 없다」로 읽는다. 그래서 이 카탈로그가 `safety.malware_hint` 에 이미 적어 둔
규율을 그대로 쓴다: **「탐지」가 아니라 「참고」다.**
...
- **실제 개인정보다** — `krrn_like`·`card_like` 의 **`_like` 는 꼴이 같다**는 뜻이다.
  Luhn 을 통과한 16자리가 실제 카드라는 뜻이 아니다
- **마스킹·비식별화 도구다** — 원문을 고쳐 주지 않는다
- **개인정보 보호 준수(컴플라이언스)** — 아무것도 보증하지 않는다

**결과가 자기 한계를 들고 다닌다.** `patterns_checked` 를 항상 같이 낸다 —
**목록에 없는 것은 「찾지 않았다」는 뜻이지 「없다」는 뜻이 아니다.** 이 칸이 없으면 빈
`findings` 가 「깨끗하다」로 읽힌다. capreq 화면도 그 문장을 그대로 보여 준다.

**원문을 결과에 담지 않는다.** 찾은 자리의 글자를 그대로 돌려주면 **결과 자체가 새 유출면**이
된다 — 결과는 증적에 남고 화면에 그려지고 로그를 탄다. 그래서 `text` 는 **가려서** 낸다
(`ops@example.dev` → `o**@e******.dev`). 대신 `start`·`end` 는 그대로 준다 —
**어디에 있었는지**는 알려 줘야 쓸모가 있고, 원문을 다시 볼 수 있는 쪽은 **원문을 가진 사람**뿐이다.

**규칙을 전부 적는다** (`app/pii_rules.py`). `krrn_like` 는 **앞 6자리가 달력에 맞아야** 한다
(없으면 임의의 13자리가 다 걸린다) · `card_like` 는 13..19자리이고 **Luhn 통과**해야 한다 ·
겹치면 먼저 온 패턴이 이긴다 (`text.ner` 과 같다) · `ipv6` 는 **축약(`::`)을 다 풀지 않아**
뒷부분만 걸릴 수 있다 — **자리를 알리는 데는 쓸모가 있고 「전체를 잡았다」고는 못 한다.**

**자르지 않고 던진다.** `NODE_MAX_PII_FINDINGS` 를 넘으면 실패한다 — 자르면 「전부 찾아봤다」가
거짓이 되고, **PII 능력에서 그 거짓말은 특히 나쁘다.**

텍스트를 읽는 능력이 다섯이 됐고, 다섯이 내놓는 것이 다르다.

| 능력 | 내놓는 것 |
|---|---|
| `text.ner` | 타입 span (`email`·`url`·`ipv4`·…) — **원문 그대로** |
| `text.extract` | `키: 값` 필드 |
| `table.extract` | 격자 (행 × 열) |
| `text.rank` | 후보 줄의 순위 |
| **`safety.pii`** | **가려진 span + 찾아본 패턴 목록** |

`text.ner` 과 겹치는 라벨이 있지만 **하는 일이 다르다** — `text.ner` 은 원문을 그대로 주고,
여기는 **가려서** 준다. 반대로 `safety.pii` 는 주민·카드·전화 꼴을 더 본다.
