...
| D24 | **조직 경계 — 등급과 소속을 분리한다.** `org` 테이블 + `app_user`·`node`·`task`·`node_invite.org_id`(`0017`). **`node.org_id IS NULL` = 팀 운영 공용 기기**로 모든 조직의 작업을 받는다. 배정은 스냅샷 2개 + 복합 FK 2개 + `ck_assignment_org`(**같은 조직이거나 공용이거나**)가 판정한다 — 행렬 테이블 없음(순서가 아니라 동일성). 조직은 **초대장에 박힌다**(G2 와 같은 모양 — 신청자가 주장하지 못한다). **Agent 는 org 를 갖지 않는다**(공용 카탈로그). 백필은 `default` org + 기존 `app_user`·`task`, **기존 Node 는 NULL 유지** · `NOT NULL` 승격 없음 | 2026-08-16. `trust_domain='tenant'` 는 **민감도 등급**이지 어느 조직이 아니다 — tenant 가 둘이면 둘 다 `'tenant'` 라 `domain_compatible` 이 구별하지 못했다. **실측: 조직 A 의 작업이 조직 B 의 기기에 배정됐다.** 조회가 아니라 **실행** 문제였고, 제품 주장(「승인하지 않은 신뢰 도메인으로 라우팅되지 않는다」)이 다중 조직에서 성립하지 않았다. `node.owner_id`·`agent.owner_id` 가 시드 admin 하드코딩이라 소유자 축도 죽어 있었다 — 같이 살렸다 |
