...
| D2 | ~~Capability = 이름이 아니라 **계약** (스키마 + 골든셋 + 게이트)~~ → **D18·D20 으로 개정 (2026-08-12)** | 원문의 「이름이 아니라 계약」은 유효하다. 폐기된 것은 **계약의 구성**이다 — 골든셋과 게이트를 계약에 **포함**시킨 부분이 D17 에서 무너졌다(하한 게이트는 대체 가능성을 보장하지 못한다). 개정 후: **계약 = 인터페이스**(입출력 스키마 · 전처리 · 실행 조건 `arch`·`max_params`), **골든셋 + 게이트 = 선택적 품질 프로파일**(`quality_profile='golden'`). `none` 인 능력도 라우팅된다 |
...
| D24 | **조직 경계 — 등급과 소속을 분리한다.** `org` 테이블 + `app_user`·`node`·`task`·`node_invite.org_id`(`0017`). **`node.org_id IS NULL` = 팀 운영 공용 기기**로 모든 조직의 작업을 받는다. 배정은 스냅샷 2개 + 복합 FK 2개 + `ck_assignment_org`(**같은 조직이거나 공용이거나**)가 판정한다 — 행렬 테이블 없음(순서가 아니라 동일성). 조직은 **초대장에 박힌다**(G2 와 같은 모양 — 신청자가 주장하지 못한다). **Agent 는 org 를 갖지 않는다**(공용 카탈로그). 백필은 `default` org + 기존 `app_user`·`task`, **기존 Node 는 NULL 유지** · `NOT NULL` 승격 없음 | 2026-08-14. `trust_domain='tenant'` 는 **민감도 등급**이지 어느 조직이 아니다 — tenant 가 둘이면 둘 다 `'tenant'` 라 `domain_compatible` 이 구별하지 못했다. **실측: 조직 A 의 작업이 조직 B 의 기기에 배정됐다.** 조회가 아니라 **실행** 문제였고, 제품 주장(「승인하지 않은 신뢰 도메인으로 라우팅되지 않는다」)이 다중 조직에서 성립하지 않았다. `node.owner_id`·`agent.owner_id` 가 시드 admin 하드코딩이라 소유자 축도 죽어 있었다 — 같이 살렸다 |
...
| **D26** | **작업량의 정본은 Core 관측이다.** `work_units` 시간 = `assignment.finished_at − created_at` (파생 · **저장하지 않는다**). `assignment.duration_ms` 는 **Node 자기신고**(추론 구간만)로 **힌트 지위**로 유지한다 — 지우지 않는 이유는 「관측 − 자기신고 = 일 밖에서 쓴 시간」이 그 둘이 있어야만 나오기 때문이다. `vram_mb_peak` · `energy_wh` 는 **미계측**으로 NULL 유지 — RSS·GPU 추정으로 채우지 않는다. 읽는 길은 `GET /v1/ops/work-units` (`developer` · read-only · 기본 최근 7일 · 상한 90일). **DDL 추가 없음** (새 컬럼 0 · 마이그레이션 0) | 2026-08-29. 브리지 `pr-c-work-units` Decision D1-(a)·D2-(a)·D3. 근거는 **절대규칙 4의 확장**이다 — Node 가 자기 등급을 주장할 수 없다면 **자기 일의 양**도 같은 질문을 받는다. 실측: 같은 배정에서 자기신고와 관측이 평균 **789 ms** 차이 났고(최대 +1982 ms), 자기신고 최소값은 0 ms 인데 관측은 수백 ms 였다. 그 값으로 원가를 세우면 전송·대기·큐가 통째로 빠진다(§8.2). 「칸 이름이 거짓말이 되게 하지 않는다」가 `vram` 을 RSS 로 채우지 않는 이유다 |
...
| **D27** | **`retrieve.*` 는 카탈로그에 선언만 두고 구현하지 않는다.** 11번째 능력 후보였던 `retrieve.dense`(#51)를 **기각**한다. 조건은 좋았다 — 이미 선언돼 있고, `step6-executors.md` §3 이 `retrieve.*` 를 「자체 생성 가능 · 라이선스 0」으로 분류하고, `text.embed` 가중치를 재사용하면 새 학습이 0 이다. **그런데 그 벡터가 의미가 아니다** — `app/text_features.py` 는 문자 n-gram 을 해시하므로 코사인은 **철자·형태 유사도**다. 「dense retrieval」은 통상 **의미 검색**을 뜻하니 이름이 실질과 어긋나고, 낱말 겹침을 쓰는 `text.rank` 와 **같은 축**이라 능력 수만 는다. **의미를 담은 임베딩이 생기기 전에는 다시 보지 않는다** — 사전학습 가중치를 못 쓰므로(D6) 가까운 시일에는 아니다. **다음 카탈로그 +1 은 「이름이 실질과 맞는가」를 먼저 본다** | 2026-09-02. 브리지 `11th-capability-reject` → ack 수령. `safety.pii` 는 이름이 위험했지만 규율을 걸 수 있었다(`_like` · `patterns_checked` · 마스킹) — `retrieve.dense` 는 **이름 자체가 기술 용어**라 「의미가 아니다」를 붙여도 「dense retrieval 이 있다」로 읽힌다. 그 차이가 기각 근거다. 후속 후보 `timeseries.anomaly` 는 이 검사를 통과해 별 Proposal 로 열려 있다(Decision 대기) |
