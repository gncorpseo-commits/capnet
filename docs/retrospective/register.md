...
### SD-017 · DB 커넥션 풀이 없다 → **해소 (2026-08-11)**
...
- **처음엔 미루자고 했다.** 촬영 12일 전 핵심 배관이고 새 의존성이라서다. **master 가 지금 하자고 결정**해서 진행했다
- **채택:** `psycopg-pool==3.3.1` (psycopg 저자들의 공식 동반 패키지 · LGPL-3.0 · THIRD-PARTY 등재)
- **위험 관리:** 호출부 시그니처·커밋 규약을 바꾸지 않았다 (`with get_conn() as conn:` 그대로).
  **풀이 없거나 못 열면 예전 방식으로 떨어진다** — `DB_POOL=0` · import 실패 두 경로 모두 실측 확인
- **효과 (같은 조건 재측정):**
  | 항목 | before | after |
  |------|--------|-------|
  | `GET /health` (DB) | 15.2ms | **3.8ms** |
  | DB 없는 요청 | 3.3ms | 3.0ms |
  | 30건/동시10 | 5.35/s · e2e 1.60s | 5.84/s · e2e **1.23s** |
  | 100건/동시25 | 5.44/s · e2e 4.51s | **12.38/s** · e2e **1.94s** |
  | 실 스택 `GET /health` | 14.6ms | **3.8ms** |
- **포화가 풀렸다** — before 는 부하를 3배 올려도 5.4/s 로 평평했는데, after 는 5.84 → 12.38 로 **늘어난다**
- **회귀:** 통합 6종 · 깨끗한 환경 재현 9/9 · 실 사슬 정상
- **남은 주의:** 풀은 **프로세스 안에서만** 공유된다. uvicorn 워커를 늘리면 `DB_POOL_MAX × 프로세스 수` 가
  postgres `max_connections` 를 넘지 않게 한다
- **상태:** closed
