...
| `prod_room.sh` (14) | ❌ **수동** | **HTTP 계층** — 강제 모드에서 무인증 401, 증서 없는 Node 401, 사칭 403 |

즉 「강제를 켜면 실제로 401 이 나온다」는 **사람이 손으로 돌릴 때만** 확인된다.
안전이 핵심 기능인데 그 회귀를 자동으로 잡지 못한다.
