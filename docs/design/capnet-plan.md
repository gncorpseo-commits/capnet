...
**채점 가능한 Capability 계약**을 요청하면, **게이트를 통과한 Agent**가 **신뢰 도메인 안의 Node**에서 Task를 **단일 노드로 완결**하는 실행 계층이다.
...
**Capability 추상화가 성립하는가** (E2E는 전제 조건).
...
| 결과 | 판정 | 다음 |
|------|------|------|
| A/B 편차 &lt; 0.05, 통과율 20–80% | **Go** | Phase 2 (테넌트 설계) |
