...
| **모델 없이도 됨** | 규칙 기반이 정직한 구현 | **`safety.pii` ✅** · **`text.ner` ✅** · **`text.extract` ✅** · **`text.rank` ✅** |

> 「모델 없이도 됨」은 이제 **추정이 아니라 실측**이다. `text.ner`(2026-08-27) ·
> `text.extract`(2026-08-29) · `text.rank`(2026-08-30) · `safety.pii`(2026-08-31) **넷 다**
> 파라미터 0 으로 계약 게이트를 통과하고 Task 를 완주했다.
