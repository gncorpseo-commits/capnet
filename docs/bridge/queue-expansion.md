...
**번호 큐가 비어도 멈추지 마. 활성은 배치 R. 시드 161+ 발명 금지. 머지를 묻지 마. 「상태확인」이면 동기화 후 즉시 R1.**
...
1. **배치 R 의 R12 Step 0** 을 남겼다 (Docker 없어 산 항목이 전부 못 봄이어도 R12면 종료)
2. **하드 블로커** — schema/CHECK/정책 숫자/제품 주장
3. 사용자가 **명시적으로 중단**

시드 12–160 + 최종 G + 「시드 종료」 Step 0 은 **이미 충족**됐다 (ACK #340). 그것을 「할 시드 없음 → 정지」로 읽지 마 — 활성은 R.
**번호·배치 소진 ≠ 종료** 는 시드 구간에 해당했다. 배치 R 소진 = R12. 「다음 배치 대기」·161+ 발명이라고 쓰지 마.

---

## 3. 큐를 스스로 늘리는 규칙

매 코드 PR 또는 Step 0마다 inbox에 **다음 줄 ≥3**을 남긴다 — **표에 이미 있는 번호**를 가리킨다.
**161+ 와 표 밖 시드를 발명하지 않는다.**
고르는 우선순위: `queue-batches.md` 배치 R 안 우선순위표. G 양산 금지.
...
| **71–100** | #266–#294 | **배치 B 완료** |
| **101–130** | #297–#314 | **배치 C 완료** |
| **131–160** | #316–#337 | **배치 D 완료** |
| **최종 G** | #338·#339 | **시드 종료** · ACK #340 |
| **배치 R** | — | **활성** → [`queue-batches.md`](./queue-batches.md) §8 |
...
**[`queue-batches.md`](./queue-batches.md)** — 배치 R 활성 · 상태확인 · 전달 문구.

---

## 6. 상시 생성기 (배치 소진 뒤 · 최종에서도 한 바퀴)
...
| 날짜 | 비고 |
|---|---|
| 2026-09-06 | 배치 R 활성 · 역할 실측 · 첫 줄 R1 |
| 2026-09-06 | C 활성 · C→D→최종 연속 · 첫 줄 #101 |
