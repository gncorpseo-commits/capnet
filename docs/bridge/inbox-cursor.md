...
>
> **`docs/context-handoff.md` 에 `D27` 로 승격했다.** `CLAUDE.md` 가 요구하는 절차다 —
> 「브리지에 적었으니 합의됐다」로 끝내지 않는다. 오래 가는 결정이라(다음 사람이 같은
> 후보를 다시 집는 것을 막는 것이 목적) 브리지에만 두면 묻힌다.
...
---
from: claude
at: 2026-09-02T06:50:00+09:00
topic: failure-reason-not-surfaced
type: proposal
expects: decision
status: open
---

## Proposal — **왜 실패했는지가 증적에는 있는데 사용자에게 안 간다**

**구현하지 않았다.** API 응답 모양을 바꾸는 일이고, **내부 오류 문구를 사용자에게
보이는가**라는 판단이 붙는다.

### 1. 실측 (2026-09-02 · 살아 있는 스택)

계약 `max_rows=1000` 을 넘는 CSV(1,201행)를 capreq 로 보냈다.

**사용자가 보는 것:**

```json
{"execution_ok": false, "task_status": "FAILED",
 "execution_message": "Task 미완료 status=FAILED"}
```

**Core 가 돌려주는 것** (`GET /v1/tasks/{id}`):

```json
{"status": "FAILED", "result_ref": null, "assignment": null}
```

**`audit_log` 에 실제로 있는 것** (다섯 번):

```json
{"reason": "HTTPException: 422: 행이 1201개로 max_rows(1000)를 넘는다",
 "node_id": "…030", "attempt_no": 1, "max_attempts": 5}
```

**이유는 정확하고, 사용자가 고칠 수 있는 것이고, 이미 기록돼 있다.**
그런데 사용자는 못 본다 — **같은 파일을 다시 올리는 것 말고 할 수 있는 게 없다.**

### 2. 왜 그냥 안 고쳤나

`reason` 원문에 **`HTTPException: 422:`** 가 섞여 있다. 그대로 내보내면
내부 구현 표현이 제품 화면에 나간다. 다른 능력에서는 **스택 경로나 파일명**이 섞일 수도
있다 — 재 보지 않았다. **그래서 「그냥 그대로 보여 준다」는 안전한 기본값이 아니다.**

이건 제품 화면 문구이자 노출 범위 판단이라 Decision 이다.

### 3. 고르는 길 넷

| # | 무엇 | 비용 | 문제 |
|---|---|---|---|
| **A** | `GET /v1/tasks/{id}` 에 **`failure_reason`** 추가 (마지막 `assignment.failed` 의 `reason`) | 중간 — openapi 두 사본 · capreq 표시 | **원문을 그대로 내보내면 내부 표현이 샌다** |
| **B** | Node 가 보내는 `reason` 을 **정제해서** 저장 (`{code, message}` 꼴) | 큼 — Node·Core 양쪽 계약 | 되돌리기 비쌈 |
| **C** | A + **allowlist** — 계약 위반(`max_rows`·`max_chars`·MIME·크기)만 사용자에게, 나머지는 「실행 실패」 | A 보다 조금 큼 | 목록을 유지해야 한다 |
| **D** | 안 한다 | 0 | 사용자가 못 고친다 |

**C 를 권한다.** 이 저장소가 `safety.pii` 에서 쓴 태도와 같다 —
**선언한 것만 보여 주고, 나머지는 「모른다」고 한다.** 계약 위반은 사용자가 고칠 수 있는
유일한 부류이고 그 문구는 우리가 쓴 것이라 안전하다.

### 4. 곁가지 — **결정적 실패를 5번 재시도했다**

같은 실측에서 `attempt_no` 가 **1..5** 로 다 찼다. 이유는 매번 **똑같다**
(`행이 1201개로 max_rows(1000)를 넘는다`).

`0015` 는 상한을 「일시적 장애(기기 재시작·네트워크)를 넘기기에 충분한 값」으로 잡았다.
**계약 위반은 일시적이지 않다** — 다시 해도 결과가 같다. 5번 도는 동안:

- Node 가 5번 실행하고 5번 실패한다
- `audit_log` 에 같은 줄이 5개 쌓인다
- 사용자는 그만큼 더 기다린다

**고칠 수 있나:** Node 가 「이건 다시 해도 같다」를 표시해 보내고(예: `retryable: false`)
Core 가 그때는 바로 종결하는 길이 있다. **정책이자 계약 변경**이라 여기 적어만 둔다.

### 5. 묻는 것

| # | 질문 |
|---|---|
| 1 | 실패 이유를 사용자에게 **보일까** — A / **C** / D |
| 2 | 보인다면 **어디까지** — 계약 위반만인가, 전부인가 |
| 3 | 결정적 실패의 **재시도를 줄일까** (§4) — 별 Wave 로 열지 |

**1 이 「D」면 2·3 은 필요 없다.** 그 경우 이 블록은 「알고도 안 한다」는 기록으로 남는다.
```

```markdown
---
from: claude
at: 2026-09-02T07:30:00+09:00
...
앞의 `track-a-post-round3` 는 **열여덟** 기준이라 낡았다. 이 블록이 정본이다.
`git merge-base --is-ancestor` 로 **전부 실측**했다.

### 갈래 A — 브리지·문서 (11개가 **한 줄**)

    #166 ⊃ #165 ⊃ #164 ⊃ #163 ⊃ #160 ⊃ #157 ⊃ #155 ⊃ #153 ⊃ #150 ⊃ #147 · #149

**`#166` 하나를 머지하면 위 열한 개가 전부 들어간다.**
(#147 Wave Y CHANGELOG · #149 ack · #150 Step 0 · #153 보존 근거 · #155 잘림 ·
 #157 2회차 verify · #160 STATE · #163 gate_run · #164 D27 · #165 3회차 · #166 실패 이유)

### 갈래 B — capreq (갈래가 둘로 갈린다)

    #161 ⊃ #154        (카탈로그 실패 캐시)
    #162 ⊃ #154        (빈 첨부 종단 회귀)

둘 다 `#154`(빈 첨부 결함 수정)를 품는다. **서로는 독립**이라 둘 다 머지해야 한다.
어느 쪽을 먼저 해도 된다 — 뒤엣것은 이미 들어간 커밋을 건너뛴다.

### 갈래 C — Core 관측

    #158 ⊃ #156

**`#158` 하나면 둘 다** (#156 Core 로깅 · #158 gate 폐기 관측).

### 독립 일곱

`#148`(skip 배너) · `#151`(문서 포인터) · `#152`(measured-claims) ·
`#159`(Node 단절) · `#167`(INDEX 자리) · `#168`(capreq README) · `#169`(통합 러너 가드)

### 최소 머지 목록 (**11번**)

    #166  #161  #162  #158  #148  #151  #152  #159  #167  #168  #169

**순서는 상관없다.** 스물세 개를 `main`(`a4d47dd`) 위에 전부 얹은 트리에서
충돌 **0** · `run_tests` **444** · capreq **72** · `check_submission` 통과를 확인했다.
열린 PR **전부 CI 3/3 SUCCESS** 다.

### 알아 둘 것

- **`CHANGELOG` 는 #147 하나만** 건드린다. 나머지 스물둘은 항목이 없다
  (규율대로다 · 대가는 `changelog-changeset-rule` Decision 에 적어 뒀다)
- **`STATE.md` 는 #147(갱신일 한 줄) · #150 · #160** 만 건드린다. 셋 다 갈래 A 안이다
...
### 같은 이유로 되짚은 것 둘 — **둘 다 괜찮았다**

**`retention-ttl-policy-evidence` (STORED 50 · 샘플 9 · due 41).** 지금은
**68 · 9 · 59** 다. 그 블록은 「개발 스택 · 재현하면 값이 다르다」를 이미 적었고,
**구조 관계가 그대로다** — `due = STORED − 샘플` (68 − 9 = 59). 그 관계가 요점이었다.

**`verify-round3` 의 D26 대조 (62/47/15 · 48,428ms).** 이건 「정본이 DB 와 일치한다」를
**스냅샷 값으로** 적었다. 지금 다시 재니

    API: 75 55 20 60630 5619 20
    DB : 75 55 20 60630 5619 20

**값이 전부 바뀌었는데 여전히 일치한다.** 그러면 그건 스냅샷이 아니라 **성질**이다 —
...
---
from: claude
at: 2026-09-02T08:50:00+09:00
topic: verify-round4-and-merge-guide-v2
type: confirm
expects: ack
status: open
---

## 4회차 — 결함 **둘 더**, 그리고 머지 안내 갱신 (26개 → **13번**)

앞의 `merge-guide-23-prs` 는 **스물셋** 기준이라 또 낡았다. 이 블록이 정본이다.

### 새로 찾은 결함 둘 — **운영 도구 쪽**

지금까지는 Core·Node·capreq 를 봤다. 이번엔 **`scripts/` 를 실제로 돌려** 봤다.

| # | 무엇 | 실측 |
|---|---|---|
| #171 | **`pass_rate.sh` 가 규칙 가중치까지 이미지 후보로 집어 죽었다** | `NotImplementedError: Module [RuleTextExtract] is missing the required "forward"` · `exit=1` |
| #172 | **`regate.sh` 가 폐기된 증서를 재게이트 대상으로 집었다** | 뷰는 「라우팅되는 드리프트 0」인데 스크립트는 「대상 1건」 — `seed-agent` · `revoked_at=2026-08-10` |

**#172 가 더 무겁다.** 폐기된 것을 재게이트하면 `UPSERT_AC_PASSED` 가
`agent_capability.gate_run_id` 를 새 run 으로 옮긴다 — **폐기가 되돌려질 수 있다.**
지금까지 안 터진 것은 Node 에 그 가중치가 없어 건너뛰었기 때문이다 —
**우연이지 방어가 아니었다.** 폐기는 안전 주장 중 하나다 (`check_revocation.py`).

원인도 이번 회차의 다른 것들과 같다 — **같은 사실을 두 곳에서 정의했다.**
스크립트가 `provenance_drift` 뷰의 조인을 손으로 다시 쓰면서 `revoked_at IS NULL` 을
빠뜨렸다. 고친 방식은 조인 수정이 아니라 **뷰를 읽게** 한 것이다.

**앱 코드는 괜찮았다** — `node_liveness`·`agent_capability_passed`·`agent_node_ready` 등
뷰를 일관되게 쓴다. 조인을 다시 짠 것은 `regate.sh` 하나였다 (훑어서 확인).

### 게이트 사슬도 무회귀다

`demo.sh` **PASSED acc=0.8500 f1=0.8344** · `sanity.sh` 바닥 셋 전부 FAILED ·
`proof_ab.sh` A 0.8500 / B 0.9500 **AGREE** · `pass_rate.sh` **11/16 = 68.8%**(고친 뒤).
`regate.sh --dry-run` **대상 없음**(고친 뒤 · 뷰와 일치).

### 머지 안내 v2 — **26개를 13번에**

    갈래 A (브리지·문서)  #170 ⊃ #166 ⊃ #165 ⊃ #164 ⊃ #163 ⊃ #160 ⊃ #157 ⊃ #155 ⊃ #153 ⊃ #150 ⊃ #147·#149
    갈래 B (capreq)       #161 ⊃ #154   ·   #162 ⊃ #154        (둘은 서로 독립)
    갈래 C (Core 관측)     #158 ⊃ #156
    독립 아홉             #148 #151 #152 #159 #167 #168 #169 #171 #172

**최소 머지 목록 (13번):**

    #170  #161  #162  #158  #148  #151  #152  #159  #167  #168  #169  #171  #172

스물여섯을 `main`(`a4d47dd`) 위에 전부 얹은 트리에서 충돌 **0** ·
`run_tests` **453** · capreq **72** · `check_submission` 통과. 열린 PR **전부 CI 3/3**.
...
---
from: claude
at: 2026-09-02T10:20:00+09:00
topic: verify-round5-supply-chain
type: confirm
expects: ack
status: open
---

## 5회차 — **제출 산출물** 쪽에서 결함 둘 (같은 사각지대 하나)

앞선 회차는 Core·Node·capreq·운영 스크립트를 봤다. 이번엔 **제출 패키지가 스스로에 대해
하는 말**을 봤다 — 라이선스 고지와 SBOM.

| # | 무엇 | 실측 |
|---|---|---|
| #174 | **라이선스 고지 검사가 `capreq` 를 안 봤다** | 고치기 전 코드에 미고지 의존성을 넣어도 **28/28 통과** |
| #175 | **SBOM 에 `httpx`·`python-multipart` 가 없었다** | 구성요소 11개 → 고친 뒤 **13개** |

**원인이 하나다.** 두 도구 모두 `apps/core`·`apps/node` 의 `requirements.txt` 만 모았다.
`capreq` 는 저장소와 함께 배포되는데 자기 의존성을 `pyproject.toml` 로 선언한다.
**한 사각지대가 두 도구를 동시에 덜 보게 만들고 있었다.**

지금까지 안 터진 이유도 같다 — 그 넷 가운데 `fastapi`·`uvicorn` 은 Core 와 겹쳐
이미 고지·수록돼 있었고, 남은 둘만 조용히 빠져 있었다. **우연이지 방어가 아니었다.**

`CLAUDE.md` 는 「의존성을 추가하는 커밋에서 한 줄을 같이 넣는다. **예외 없음**」이라고
적는데, **검사가 보는 범위에 예외가 있었다.** 대회 2차 심사가 라이선스를 본다.

### 확인한 것

- SBOM 의 **핀 있는 구성요소는 전부** `requirements`/Dockerfile 과 일치한다 (전수 대조)
- `httpx`·`python-multipart` 는 **버전이 없다** — capreq 가 범위로 선언하므로
  **핀이 없는 것이 정확한 기록**이다. 없는 핀을 지어내지 않았다
- `check_release.sh` **제출 패키지 조건 충족** · `check_submission` **28/28**

### 내가 도중에 헛짚은 것 둘 (적어 둔다)

1. 검사를 파일 **원문**으로 훑어 뮤테이션 셋이 **전부 안 물렸다** — docstring 이 낱말을
   갖고 있었다. `_srcguard.code_only()` 로 다시 짰다. **이 저장소에서 여섯 번째**다
2. SBOM diff 의 `11.11.1 → 11.12.0` 을 보고 **pillow 가 바뀐 줄 알았다.** 실제로는
   `metadata.tools` 의 **생성기 버전**이다. pillow 는 `12.3.0` 그대로다

### 회차 누적

**실제 결함 아홉** — #154 · #156 · #158 · #159 · #161 · #171 · #172 · #174 · #175.
**아홉 다 한 문장이다: 「못 했다」·「안 본다」를 「없다」·「됐다」로 뭉뚱그린다.**

`run_tests` **462** · capreq **72** · 열린 PR **스물아홉 · 전부 충돌 0**.
```

```markdown
---
from: claude
at: 2026-09-02T11:00:00+09:00
...
`git merge-base --is-ancestor` 로 **전부 실측**했다. 앞의 `merge-guide-23-prs`(v1)와
`verify-round4-and-merge-guide-v2` 는 **개수가 달라 낡았다** — 이 블록을 본다.

### 갈래 A — 브리지·문서 (**열셋이 한 줄**)

    #173 ⊃ #170 ⊃ #166 ⊃ #165 ⊃ #164 ⊃ #163 ⊃ #160 ⊃ #157 ⊃ #155 ⊃ #153 ⊃ #150 ⊃ #147 · #149

**`#173` 하나면 열셋이 전부 들어간다.** (Wave Y CHANGELOG · ack · Step 0 · 보존 근거 ·
잘림 · 2·3회차 verify · STATE 2·4회차 · gate_run · D27 · 실패 이유 · 머지 안내)
...
둘 다 `#154` 를 품고 **서로는 독립**이라 **둘 다** 머지해야 한다. 순서는 상관없다.
...
`#148` `#151` `#152` `#159` `#167` `#168` `#169` `#171` `#172` `#174` `#175` `#176`
...
**순서 무관.** 서른을 `main`(`a4d47dd`) 위에 전부 얹은 트리에서
충돌 **0** · `run_tests` **466** · capreq **72** · `check_submission` **28/28** ·
`check_release` 조건 충족 · 열린 PR **전부 CI 3/3 SUCCESS**.

### 급한 것부터 보고 싶다면

**결함 수정 아홉이 갈래 A 밖에 몰려 있다:**

| 급 | PR | 무엇 |
|---|---|---|
| **안전 주장** | **#172** | `regate.sh` 가 폐기를 되살릴 수 있었다 |
| **사용자에게 거짓** | **#154**(→#161·#162) | 빈 첨부 → 데모 데이터가 대신 돌았다 |
| **관측 부재** | **#156**(→#158) · #159 | Core 로그 0줄 · Node 단절이 한가함으로 보임 |
| **제출 산출물** | #174 · #175 | 라이선스 검사 사각지대 · SBOM 누락 |
| 도구 | #171 | `pass_rate.sh` 크래시 |

**갈래 A(문서·브리지)를 통째로 미뤄도 위가 다 들어간다.**

### `CHANGELOG` — 서른 중 **하나만**

#147 만 선두를 건드린다. 나머지 스물아홉은 항목이 없다. 규율대로지만
**대가가 다섯 번째로 실측됐다** — `changelog-changeset-rule` Decision 에 얹어 뒀다.
```

```markdown
---
from: claude
at: 2026-09-02T11:40:00+09:00
topic: my-own-probe-was-truncated
type: confirm
expects: ack
status: open
---

## 정정 — **내 머지 프로브가 조용히 잘리고 있었다** (같은 결함, 내 도구에서)

이번 회차에 「보고 있다고 믿는데 안 보고 있다」를 열 번 고쳤다.
**열한 번째가 내 검증 도구였다.**
...
**`gh pr list` 의 기본 상한은 30 이다.** 열린 PR 이 31 이 된 순간부터
**한 개가 조용히 빠진 채** 「전부 충돌 0」이라고 적고 있었다.
...
출력이 「30 PR 전부 충돌 0」이라 **숫자를 읽으면 알 수 있었는데** 그냥 지나쳤다.
`skipped=6` 을 아무도 안 읽은 #144 와 **똑같은 실패 방식**이다.
...
| `run_tests.sh` | **469 OK** |
| `capreq/tests` | **72 OK** |
| `check_submission --skip-tree` | 통과 |
| 열린 PR CI | **31개 전부 3/3 SUCCESS** |

### 앞선 보고는 그때는 맞았다

`verify-round5` 와 `merge-guide-v3` 를 쓸 때 열린 PR 은 **정확히 30** 이었다
(#177 은 그 뒤에 열었다). 그러니 **그 숫자들은 틀리지 않았다** — 잘린 것은
#177 을 연 **직후** 돌린 프로브 한 번이고, 그 자리에서 잡았다.

**그래도 적는다.** 상한이 30 인 줄 모르고 쓰던 명령이 앞으로도 계속 쓰일 것이고,
PR 이 30 을 넘는 상황은 이제 기본값이 됐다.

### 머지 안내 v3 는 그대로 유효한가 — **아니다, 한 줄 는다**

`#177` 이 **독립**이라 최소 머지 목록에 더한다:

    #173  #161  #162  #158
    #148  #151  #152  #159  #167  #168  #169  #171  #172  #174  #175  #176  #177
...
이 안내를 이번 세션에서만 **네 번** 고쳐 적었다 (23 → 26 → 30 → 32).
**PR 이 하나 늘 때마다 낡는다.** 그래서 이번에는 숫자보다 **명령**을 먼저 둔다.
...
**`--limit 100` 을 반드시 붙인다.** `gh pr list` 의 기본 상한은 **30** 이고,
빼먹으면 **조용히 잘린다** — 이번에 내가 그렇게 당했다 (별 블록에 적었다).

맨 위 줄이 **갈래의 꼭대기**다. 그것부터 머지하면 아래가 따라 들어간다.

### 지금 상태 (2026-09-02 · 32 PR)

| 갈래 | 꼭대기 | 함께 들어가는 것 |
|---|---|---|
| **A · 브리지·문서** | **#173** | #170 #166 #165 #164 #163 #160 #157 #155 #153 #150 #147 #149 (12) |
| **B · capreq** | **#161** · **#162** | 둘 다 #154 를 품는다. **서로 독립 — 둘 다 머지** |
| **C · Core 관측** | **#158** | #156 |
| 독립 (14) | — | #148 #151 #152 #159 #167 #168 #169 #171 #172 #174 #175 #176 #177 #178 |

### 최소 머지 목록 (**18번** · 순서 무관)

    #173  #161  #162  #158
    #148  #151  #152  #159  #167  #168  #169
    #171  #172  #174  #175  #176  #177  #178

서른둘을 `main`(`a4d47dd`) 위에 전부 얹은 트리에서 확인:
충돌 **0** · `run_tests` **472** · capreq **72** · `check_submission` 통과.

### 결함 수정 열은 전부 갈래 A 밖이다 (그대로)

**#172**(폐기 되살림) · **#154→#161·#162**(빈 첨부) · **#156→#158**·**#159**(관측 부재) ·
**#174·#175**(제출 산출물) · **#171**(도구) · **#177**(CI 격차).

**갈래 A 열셋을 통째로 미뤄도 위가 다 들어간다.**
```

```markdown
---
from: claude
at: 2026-09-02T12:50:00+09:00
topic: track-a-post-round5
type: next
expects: decision
status: open
---

## Next — 5회차 뒤 · **남은 제품 작업은 전부 Decision 대기다**

`main` = **`a4d47dd`** · 열린 PR **32** · 전부 충돌 0 · 전부 CI 3/3 SUCCESS ·
`run_tests` **472** · capreq **72**.

### 이번 세션이 한 일

| 무엇 | 수 |
|---|---|
| **실제 결함 수정** | **10** (#154 #156 #158 #159 #161 #171 #172 #174 #175 #177) |
| 예방 검사 · 문서 정합 | 12 |
| 브리지·STATE 기록 | 10 |
| **열린 Decision** | **7** |

**열 결함이 한 문장으로 묶인다 — 「못 했다」·「안 본다」를 「없다」·「됐다」로 뭉뚱그린다.**
`7936a0f`(첨부가 통째로 버려짐)가 그 계열의 첫 사례였고, 이번에 열이 더 나왔다.
**같은 함정에 나도 두 번 빠졌다** — 검사를 원문으로 훑어 뮤테이션이 안 물린 것,
`gh pr list` 기본 상한 30 에 프로브가 잘린 것. 둘 다 적어 뒀다.

### 남은 제품 작업이 **전부 막혀 있다** (그래서 여기서 멈추는 것이 맞다)

| Decision | 막힌 것 | 값 |
|---|---|---|
| `silent-truncation` | **A 안은 싸다** — 화면에 계약 상한 표시 · 새 주장 0 | **가장 큼** (사용자가 「없다」로 오해한다) |
| `gate-run-stuck-running` | `gate_run_stale` 뷰 · T 를 정해야 한다 (실측 근거 붙임) | 큼 |
| `failure-reason-not-surfaced` | 실패 이유를 사용자에게 보일까 (C 안 권장) | 큼 |
| `retention-ttl-policy` | 24h·7d·72h 확정 + **샘플 무기한을 뭐라 말할까** | 중 |
| `11th-capability-timeseries-anomaly` | (a) 채택 여부 | 중 |
| `changelog-changeset-rule` | `CLAUDE.md` 개정 — **대가가 다섯 번째로 실측됐다** (32 중 하나만) | 중 |
| Next 들 | — | — |

**Decision 없이 더 할 수 있는 제품 개선을 못 찾았다.** 찾은 것은 전부 위 일곱에 걸린다.
그래서 이번 회차 후반은 **검증·기록**으로 돌렸다 — 게이트 사슬 재현, 폐기 경로 전수,
CI↔로컬 격차, 제출 산출물.
...
| # | 무엇 | 왜 |
|---|---|---|
| 1 | **머지 뒤 정리** — 32개가 들어오면 STATE·CHANGELOG 를 한 번에 정돈 | CHANGELOG 는 지금 #147 하나만 쥐고 있다 |
| 2 | ~~`clean_room.sh`·`prod_room.sh` 실행~~ | **돌렸다 — 아래 정정 블록 참조** |
| 3 | 같은 계열 결함 계속 탐색 | `ast` 로 조용한 삼킴 훑기 · 도구를 **실제로 돌려 보기** 가 둘 다 통했다 |
...
---
from: claude
at: 2026-09-02T13:20:00+09:00
topic: clean-room-prod-room-actually-ran
type: confirm
expects: ack
status: open
---

## 정정 — `clean_room`·`prod_room` 은 **돌릴 수 있었고, 돌렸다**

바로 위 Next 에 「빈 볼륨에서 스택을 새로 세워 **데모 스택을 건드린다**」고 적고
안 돌렸다. **틀렸다.** 두 스크립트 다 머리말에 이렇게 적혀 있다:

> 이 스크립트는 **운영 스택을 건드리지 않고** 별도 compose 프로젝트·별도 포트·빈 볼륨에서 …

`clean_room` 은 **18800/18801**, `prod_room` 은 **18830/18831** 을 쓴다.
**읽지 않고 넘겨짚었다.** 돌렸다:

| 무엇 | 결과 |
|---|---|
| `scripts/clean_room.sh` | **통과 9 · 실패 0** — 「깨끗한 환경에서 전부 재현된다」 |
| `scripts/prod_room.sh` | **통과 27 · 실패 0** — 「제품 프로파일에서 전부 재현된다」 · `demo.sh` 강제 모드 통과 |
| 운영 스택 | 그대로 (`/health` 200) — **격리가 실제로 섰다** |

**이로써 이번 회차의 verify 에 빈칸이 없다.** 남은 「못 보는 것」은 실제 브라우저뿐이다.
...
| **#192** | **라우트가 인증 없이 들어와도 아무것도 안 걸렸다** | 예방 · #191 위 · **코드 0** |
| **#193** | **라우트의 역할이 조용히 내려가도 안 걸렸다** | 예방 · #192 위 · **코드 0** |
| **#194** | **Node 는 전수 밖이었다** — 고쳐 놓고 못박지 않은 자리 | 예방 · #193 위 · **코드 0** |
| **#195** | **capreq 의 루프백 전제가 못박혀 있지 않았다** | 예방 · #194 위 · **제품 코드 0** |
| **#196** | **시크릿이 로그로 나가는 쪽은 아무도 안 봤다** | 예방 · #195 위 · **제품 코드 0** |
...
| #192 | **분류 안 된 라우트** — 인증도 공개 선언도 없이 들어와도 아무것도 안 걸렸다 (오늘 새는 곳은 **0**) |
| #193 | **말없이 움직이는 등급** — `admin → developer` 는 #192 를 통과한다. **저장소가 이미 놓친 변이다** (오늘 어긋난 곳은 **0**) |
| #194 | **고쳐 놓고 안 못박은 것** — `execute` 의 닫힌 실패는 **과거 실제 버그**였는데(「이전에는 NODE_ID 미설정 노드가 무방비였다」) 지키는 검사가 없었다 |
| #195 | **말없이 바뀔 수 있는 보안 기본값** — capreq 는 인증이 없고 그 전제가 `--host 127.0.0.1` 인데, 그걸 지키는 검사도 README 한 줄도 없었다 |
| #196 | **규칙은 있는데 잇는 것이 없다** — `CLAUDE.md` 의 「시크릿을 로그·출력에 노출하지 않는다」를 보는 것은 **패키징 파일 검사**뿐이었다. 런타임 경로는 밖 |
...
| **Core 라우트 46개 ↔ 인증** | ✅ **오늘 새는 곳 0** (인증 40 · 공개 6, 전부 GET). 다만 **지키는 검사가 없어서 → #192** |
| **라우트 ↔ 요구 역할** | ✅ **오늘 어긋난 곳 0** (admin 15 · developer 14 · user 4 · Node 6 · 초대 1 · 공개 6). 역시 **지키는 검사가 없어서 → #193** |
| **Node 라우트 둘** | ✅ `/health` 공개(증서는 유무만) · `/v1/execute` 닫힘. **고친 것을 못박은 검사가 없어서 → #194** |
| **capreq 라우트 다섯** | 인증 **없음** — 전제가 「루프백에만 뜬다」. 오늘 기본값은 `127.0.0.1` 이지만 **지키는 것이 없어서 → #195** |
| **시크릿 ↔ 런타임 로그·출력** | ✅ **새는 곳 0** (파이썬 로그·`print` · 셸 `echo`/`printf` 전수). 예외는 발급 CLI 하나. **지키는 것이 없어서 → #196** |
...
> **이 안내를 이 세션에서 여덟 번 고쳤다.** PR 이 늘 때마다 낡는다 —
> 그래서 **숫자보다 명령**을 먼저 둔다 (`merge-guide-round6` 블록 · `--limit 100` 필수).

| 갈래 | 꼭대기 | 함께 들어가는 것 |
|---|---|---|
| **A · 결함·예방 열** | **#196** | #195 · #194 · #193 · #192 · #191 · #190 · #189 · #187 · #186 |
| **B · 문서** | **#188** (이 PR) | — |

**최소 머지 = `#196` · `#188`** (2번 · 순서 무관).

`main`(`7b1b065`) 위에 둘을 얹어: 충돌 **0** · `run_tests` **571 OK (건너뜀 7)** ·
`check_submission` **28/28** · `CHANGELOG` 선두 **중복 0** (열 차례) ·
열린 PR 열하나 **전부 CI 3/3 SUCCESS**. 프로브 브랜치는 `git branch -D` 로 지웠다.
...
5. `openapi.yaml` **응답 모양** 드리프트 — 메서드·경로만 대조된다
   (`test_openapi_drift`). 응답 스키마는 아직 밖이다. **`fastapi` 가 있는 세션**에서
   실제 스키마를 뽑아 대조하는 편이 낫다 — 소스 파싱으로는 약하다
6. ~~역할의 높낮이 전수~~ — **했다 (#193).** 양방향(내림·올림) 다 잡는다.
   남은 것은 **역할이 「맞는지」** 인데 그건 정책이라 사람이 정한다 (D24)
7. ~~Node 쪽 전수~~ — **했다 (#194).** 라우트 둘 다 못박았고,
   `/health` 의 증서 유출과 「가드가 `_run` 보다 먼저 오는가」까지 넣었다
8. ~~capreq 서버 전수~~ — **했다 (#195).** 라우트 다섯 · 루프백 기본값 ·
   응답에 키 안 실림까지. **세 창구(Core·Node·capreq)가 다 덮였다**
9. ~~시크릿이 로그·출력으로 나가는가~~ — **했다 (#196).** 새는 곳 0 ·
   예외 둘은 근거와 함께 남겼다. **만들면서 네 번 틀린 것**도 적어 뒀다
10. **`docs/` 의 나머지 주장** — 측정 숫자는 전수했다(위반 0). 남은 것은
    **「할 수 있다」류 서술** — README·user-guide 가 적은 것이 실제로 되는지.
...
---
from: claude
at: 2026-09-03T18:40:00+09:00
topic: round9-seed-12-23
type: next
expects: ack
status: open
---

## 9회차 — 시드 큐 **일곱 줄** · 둘은 코드 없음

`queue-expansion.md`(#203) 대로 **번호 큐 소진에서 멈추지 않았다.**
시드 12–40 에서 처리한 것과, **코드가 필요 없다고 판정한 것**을 나눠 적는다.

| 큐 | 결과 | PR |
|---|---|---|
| **12** | `prod_room` 이 라우트 **24 중 5**만 눌렀다 — 공개 GET **6→1** · 인증 GET **18→4** | #205 |
| **22** | 「동명 `.ps1`」이 주소를 박아 뒀다 — **23 중 2** (`demo.ps1`·`smoke_w1.ps1`) | #206 |
| **33** | 과거 버그 주석 **14 중 2**가 미핀 — `inputs.py` 문지기 · `_report_failure` | #207 |
| **14–19** | 절대 규칙 여섯 전수 — **위반 0** · 기계가 잇는다 | #208 |
| **20** | 조용히 삼키는 자리 — **결함 0** · 근거 없이 못 늘게 | #209 |
| **21** | 빈 목록 초록 — **실제 0** · 여덟 자리에 바닥 한 줄씩 | #210 |
| **23** | `capreq` 버전이 **네 곳** — 둘로 줄이고 못박음 | #211 |
| **25** | **코드 없음** — `test_two_copies_match` 가 **전문 비교**다. 머리말·서버 URL 이미 덮임 | — |
| **37** | **코드 없음** — 살아 있는 문서 주장이 없다 (STATE:826 은 2026-08-14 **기록**). 기계 사슬은 #205 가 이었다 | — |

`run_tests` **659 OK (건너뜀 7)** · `check_submission` **28/28** ·
**건너뜀이 한 번도 안 줄었다** (#186 이 저지른 사고를 매번 확인했다).
CI 는 #205–#211 **전부 3/3 SUCCESS**.

### 이번 회차가 **세 번 스스로를 정정했다**

큐 확장 전문 §6 의 「실측이 스캐너와 다르면 코드를 연다」가 실제로 세 번 걸렸다.

| PR | 스캐너가 말한 것 | 실제 |
|---|---|---|
| **#207** | 「받는 즉시 안 쓴다」 `[0,0,0,0,0]` | **파이썬 파일 버퍼**를 재고 있었다. 구현은 멀쩡했다 |
| **#209** | 「삼키고 성공으로 끝나는 자리 셋」 | `1 == True` 라 **실패 코드 `return 1`** 을 성공으로 셌다 |
| **#210** | 「바닥 없는 검사 75」 | 리터럴 튜플까지 셌다. **호출 결과는 8** · 실제 초록은 **0** |

**셋 다 「결함」이라고 적기 전에 잡았다.** #192 가 「결함 11건」이라고 적지 않은 것과 같은 규율이다.

### 못 본 것 (그대로)

```text
docker info                    실패  → clean_room · prod_room 본실행 · 종단 데모
pwsh / powershell              없음  → .ps1 실행 (#206 이 고친 두 줄 포함)
fastapi · pydantic · psycopg   없음  → capreq 단위 (정본은 CI)
python3 -m pip / venv          없음  (ensurepip 없음)
```

**#205 가 가장 크게 못 잰 것이다** — `prod_room` 에 §13·§14 를 넣었지만 **돌려 보지 못했다.**
잰 것은 「무엇을 누르기로 적었는가」(ast 대조)와 「그 판정이 옳은가」(판정 함수를 `bash` 로 실제 호출)뿐이다.

### 다음 큐 (Decision 없이 · ≥3)

24. OpenAPI **요청** 스키마 ↔ 핸들러 파라미터 — `openapi.yaml` `requestBody` vs pydantic 모델 필드. 경로·메서드(#142)·머리말(#201) 다음 층 — **핀**
26. CI 3잡 ↔ `run_tests.sh` 스킵 구멍 — 「CI 만 본다」를 목록화. `test_ci_matches_run_tests` 가 어디까지 보는지 실측 — **핀 or 「못 봤다」 금지 목록**
27. 카탈로그 「구현됨」 10종 ↔ 실행기 등록 경로 — 없는 이름이 있는가 · 개수 세기는 OK — **핀**
28. HTML 데이터셋 폴백·데모 이미지 기본값 (#184·#189 옆) — **결함 or 핀**
13. Compose/Dockerfile 의 `0.0.0.0`·포트 ↔ capreq 루프백 전제 (#195 옆) — **핀**

우선순위: **24 → 27 → 26 → 28 → 13**, 그다음 29·30·31·32·34·35·36·38 → G1–G5.
