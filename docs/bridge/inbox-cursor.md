...
> **이 안내를 이 세션에서 일곱 번 고쳤다.** PR 이 늘 때마다 낡는다 —
> 그래서 **숫자보다 명령**을 먼저 둔다 (`merge-guide-round6` 블록 · `--limit 100` 필수).

| 갈래 | 꼭대기 | 함께 들어가는 것 |
|---|---|---|
| **A · 결함·예방 아홉** | **#195** | #194 · #193 · #192 · #191 · #190 · #189 · #187 · #186 |
| **B · 문서** | **#188** (이 PR) | — |

**최소 머지 = `#195` · `#188`** (2번 · 순서 무관).

`main`(`7b1b065`) 위에 둘을 얹어: 충돌 **0** · `run_tests` **563 OK (건너뜀 7)** ·
`check_submission` **28/28** · `CHANGELOG` 선두 **중복 0** (아홉 차례) ·
열린 PR 열 **전부 CI 3/3 SUCCESS**. 프로브 브랜치는 `git branch -D` 로 지웠다.
...
9. **시크릿이 로그·출력으로 나가는가** — `check_submission.check_secrets` 는
   **패키징 대상 파일**을 훑는다. **런타임 로그**(`logger.info`·`print`)에
   키·증서가 실리는지는 아직 안 봤다
```
