...
|---|---|---|
| `main` HEAD | **`6179f42`** | `git log -1 --oneline` |
| 열린 PR | **5** (#179–#183) | `gh pr list --state open --limit 100` |
| `main` 의 `run_tests` | **472 OK (건너뜀 7)** | `bash scripts/run_tests.sh` |
| **다섯을 다 얹은 트리** | **485 OK (건너뜀 7)** | 같은 명령 · 아래 머지 프로브 |
| `check_submission` | **28/28** | 위 배너 안 (양쪽 다) |
| 열린 PR CI | **다섯 전부 3/3 SUCCESS** | `gh pr checks <n>` |

**어디서 잰 숫자인지 같이 적는다.** `485` 는 `main` 의 값이 아니다 —
세 결함 PR 이 검사 **13건**을 더한다 (#180 +7 · #181 +3 · #183 +3).
...
**셋이 한 문장이다 — 「0건인데 됐다고 말한다」.**

| PR | 0건인 것 | 그런데 뭐라 했나 |
|---|---|---|
| #180 | 매니페스트를 **하나도 안 봤다** | 「겹침 없음. 골든셋은 홀드아웃이다」 `exit=0` |
| #181 | 통합 검사를 **하나도 못 찾았다** | 「통과 0 · 실패 0」 `exit=0` |
| #183 | `UPDATE` 가 **한 행도 안 바꿨다** | `{"storage_state":"STORED", …, "purged_now":true}` |

지난 회차의 열과 같은 계열인데, **#180·#181 은 검사 도구 자신**이 그랬고
**#183 은 제품 API** 다. #183 의 「다른 쪽」은 같은 프로세스의 **GC 배경 스레드**라
경쟁이 구조상 가능하다 — 다만 바이트는 어느 쪽이든 지워지므로 **데이터 피해는 없고
거짓말한 것은 응답**이다. 과장하지 않는다.

**다섯 PR 전부 CI 3/3 SUCCESS** (마지막 확인 시점).
...
### 다음 큐 셋 (Decision 없이 되는 것)

1. **`clean_room`·`prod_room`·종단 데모** — Docker 가 되는 세션에서 돌린다. 이번에 못 봤다
2. **`call.html` 의 데이터셋 폴백** — `/v1/datasets` 실패 시 하드코딩 `eurosat-rgb` 를
   서버가 준 것처럼 보여 준다. 바로 위 줄(`c-cap`)은 에러를 보여 주는데 **같은 함수 안에서
...
### 지금 상태 (2026-09-03 · **5 PR**)

> **갱신.** 이 블록을 쓴 뒤 **#183** 이 늘었다 (`purge` 가 0행에 「지웠다」).
> 갈래 A 의 꼭대기가 **#181 → #183** 으로 옮겨졌다. **다시 쟀다** — 아래가 정본이다.

실측 결과 그대로:

```text
2  toma/purge-claims-it-purged          <- toma/integration-runner-zero-checks
                                           toma/leakage-check-cannot-claim-clean
...
| 갈래 | 꼭대기 | 함께 들어가는 것 |
|---|---|---|
| **A · 결함** | **#183** (purge 가 0행에 「지웠다」) | **#181** (통합 러너 0건) · **#180** (누출 검사 0건) |
...
|---|---|
| 충돌 | **0** |
| `run_tests` | **485 OK (건너뜀 7)** |
| `check_submission` | **28/28** |
| `CHANGELOG` 선두 | **중복 0** — #183 → #181 → #180 순으로 세 항목이 차례로 선다 |
| 열린 PR 다섯 | **전부 CI 3/3 SUCCESS** |

`_probe2` 브랜치로 재고 **지웠다** (`git branch -D`).

### `CHANGELOG` — 다섯 중 **셋**이 선두를 건드린다

#180 · #181 · #183 이 각각 한 항목씩 쓴다. **규율대로 갈라 쌓았기 때문에** 충돌이 없다 —
#181 을 #180 위에, #183 을 #181 위에 얹어 세 항목이 차례로 서게 했다.
...
대신 갈래 안의 순서가 고정되므로 **아래 것을 따로 되돌리기 어렵다.** 그 값도 같이 적는다.
