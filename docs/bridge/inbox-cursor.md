...
> **이 안내를 이 세션에서 네 번 고쳤다** (셋 → 다섯 → 여섯 → 일곱).
> PR 이 늘 때마다 낡는다. 다시 계산하는 명령은 `merge-guide-round6` 블록에 있다
> (`--limit 100` 필수).

| 갈래 | 꼭대기 | 함께 들어가는 것 |
|---|---|---|
| **A · 결함·예방 여섯** | **#192** | #191 · #190 · #189 · #187 · #186 |
| **B · 문서** | **#188** (이 PR) | — |

**최소 머지 = `#192` · `#188`** (2번 · 순서 무관).
...
|---|---|
| 충돌 | **0** |
| `run_tests` | **538 OK (건너뜀 7)** |
| `check_submission` | **28/28** |
| `CHANGELOG` 선두 | **중복 0** — #192→#191→#190→#189→#187→#186 |
| 열린 PR 일곱 | **전부 CI 3/3 SUCCESS** |
...
6. **역할의 높낮이** 전수 — #192 는 「인증을 거치는가」만 본다.
   `admin` 이어야 할 자리가 `developer` 로 내려간 것은 아직 개별 검사뿐이다
```
