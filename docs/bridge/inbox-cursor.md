...
> **이 안내를 이 세션에서 다섯 번 고쳤다** (셋 → 다섯 → 여섯 → 일곱 → 여덟).
> PR 이 늘 때마다 낡는다. 다시 계산하는 명령은 `merge-guide-round6` 블록에 있다
> (`--limit 100` 필수).

| 갈래 | 꼭대기 | 함께 들어가는 것 |
|---|---|---|
| **A · 결함·예방 일곱** | **#193** | #192 · #191 · #190 · #189 · #187 · #186 |
| **B · 문서** | **#188** (이 PR) | — |

**최소 머지 = `#193` · `#188`** (2번 · 순서 무관).
...
|---|---|
| 충돌 | **0** |
| `run_tests` | **545 OK (건너뜀 7)** |
| `check_submission` | **28/28** |
| `CHANGELOG` 선두 | **중복 0** — #193→#192→#191→#190→#189→#187→#186 |
| 열린 PR 여덟 | **전부 CI 3/3 SUCCESS** |
...
7. **Node 쪽에도 같은 전수** — Core 라우트는 #192·#193 이 덮었다.
   `apps/node/app/main.py` 의 `/v1/execute` 는 `_my_assignment` 로 닫혀 있지만
   **전수해서 못박은 적은 없다**
```
