...
> **이 안내를 이 세션에서 여섯 번 고쳤다.** PR 이 늘 때마다 낡는다 —
> 그래서 **숫자보다 명령**을 먼저 둔다 (`merge-guide-round6` 블록 · `--limit 100` 필수).

| 갈래 | 꼭대기 | 함께 들어가는 것 |
|---|---|---|
| **A · 결함·예방 여덟** | **#194** | #193 · #192 · #191 · #190 · #189 · #187 · #186 |
| **B · 문서** | **#188** (이 PR) | — |

**최소 머지 = `#194` · `#188`** (2번 · 순서 무관).
...
|---|---|
| 충돌 | **0** |
| `run_tests` | **556 OK (건너뜀 7)** |
| `check_submission` | **28/28** |
| `CHANGELOG` 선두 | **중복 0** — #194→#193→#192→#191→#190→#189→#187→#186 |
| 열린 PR 아홉 | **전부 CI 3/3 SUCCESS** |
...
8. **capreq 서버**(`capreq/src/capreq/server.py`)에도 같은 전수 — 아직 안 봤다.
   `fastapi` 가 필요해 이 환경에서는 `ast` 로만 볼 수 있다
```
