...
띄운 capreq 가 기본 `:8000` 을 보고, 스크립트는 그걸 **「배선이 끊겼다」(exit 1)** 로 보고했다 —
1(배선)/2(라우팅) 규약이 있는 스크립트에서 환경 전달 누락이 1 로 샌 것이다. 주소와 관리 키를 같이 넘긴다.
넘겨준 뒤 같은 명령이 exit 0 으로 완주(라우팅 `text.ner@1` · Core 중개 `input_id` · 빈 첨부 거절).
뮤테이션 2/2 (전달 삭제 · 주석으로 이동) 운다.

```bash
python3 -m unittest tests.test_capreq_demo_hands_over_the_core_url
```

## 활성 배치가 R 을 못 읽으면 「상태확인」이 시드 종료에서 멈춘다 — 2026-09-06

시드 12–160 · 최종 G 는 끝났다. 다음 일은 역할 경로 실측(배치 R)인데, `#155` 검사가 활성 행을 A–D·「최종」만 봐서
`배치 R` 을 켜면 검사가 깨지거나 — 더 나쁘게 — 옛 「최종」을 계속 가리킨다. 정규식에 R 을 넣고 STATE 첫 「다음:」과
같은 글자를 요구한다. 뮤테이션 2/2 (`[A-DR]` → `[A-D]` · STATE 첫 「다음:」에서 「배치 R」 삭제) 운다.
...
| 문서의 숫자 | 실물 |
|---|---|
| 마이그레이션 18개 적용 (체크리스트) | 18 ✅ |
| 위반 6종 · ×6 · 6건 (둘 다) | `demo_violations.sql` NOTICE 6 ✅ |
| 위반 14종 실측 표 | `pg-violations.md` 행 14 ✅ |
| sanity 3종 (둘 다) | `sanity.sh` 모드 3 ✅ |
| 데모 골든 N=40 (런북) | v1 manifest 40 ✅ |
| 프로브 라우트 5 → 24 (둘 다) | `prod_room.sh` 무인증 단건 6 + 루프 18 = 24 ✅ |
| 능력 **6종** 데모 재현 (체크리스트 169) | 카탈로그 **9** — 당시 기록이라 지우지 않고 「지금은 9종」을 옆에 붙였다 |

`tests/test_ops_docs_counted_claims.py` 가 일곱을 실물에서 세어 고정. 뮤테이션 4/4 (18→17 · N=40→50 ·
sanity 모드 하나 제거 · 5→24 를 5→22) 운다.
...
나머지를 못 늘린 이유를 적었다 — 「라우트마다 유효한 최소 몸통이 필요하고, 그게 맞는지는
**돌려 봐야** 안다」.

**「몸통이 무엇인가」는 돌려 보지 않아도 안다.** 요청 모델이 소스에 있다.
...
| **필수 필드가 있는 것** | **10** |
| 몸통 없이 눌러도 **401 까지 가는** 것 | **12** |

**`#235` 를 정정한다.** 「열아홉 전부 몸통이 필요하다」로 읽힐 수 있게 적었는데, 실제로는
**열 개**만 필요하다. 그만큼 Docker 회차의 일이 줄어든다.

### 표가 낡으면 운다

모델에 필수 필드가 하나 늘면 표의 몸통이 부족해지고, 그 프로브는 **401 대신 422** 를
받는다 — `#223` 이 `node_id` 로 겪은 그 함정이다. 검사가 모델과 표를 대조한다.

### 뮤테이션 3

| 심은 것 | 운 검사 |
|---|---|
| 모델에 필수 필드 추가 (`NodeRedeem.org_code`) | `…has_the_models_required_fields` |
| 표에서 필수 필드 삭제 | 같은 검사 |
| 필요 없는 라우트에 몸통 추가 | `test_no_body_is_listed_for_a_route_that_needs_none` |

**응답은 안 쟀다** — 401 이 나오는지는 강제 모드에서 `prod_room` 이 잰다 (`docker info`
실패 · 규약 6). 여기는 **몸통이 계약과 맞는가**만 본다.

재현: `python3 -m unittest tests.test_write_route_minimal_bodies` (7 검사 · 991 통과)
