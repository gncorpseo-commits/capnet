...
| 화면 | 무엇 | 관리 키 |
|---|---|---|
| **Node** (`/ui/nodes.html`) | 등록 · 함대 상태(생존·증서) · 증서 발급/폐기 | 필요 |
| **능력 호출** (`/ui/call.html`) | Capability 로 요청 · 결과와 **증적** · dummy 경고 | 필요 |
| **초대 발행** (`/ui/invite.html`) | 초대장 발행·목록·폐기 · 토큰/링크 복사 | **admin** |
| **초대 소진** (`/ui/redeem.html`) | 초대로 기기 등록 + 증서 수령 | **불필요** |

외부 자산(CDN·폰트)을 쓰지 않는다 — 내부망·오프라인에서 그대로 뜬다. 새 의존성도 없다.
UI 가 못 하는 것은 **Agent 게이트·바인딩**이다 (가중치가 기기에 있어야 하므로 터미널 작업).

> **강제 모드에서도 쓸 수 있다.** 화면 위쪽 **관리 API 키 입력줄**에 키를 넣으면 된다.
>
> 키는 **브라우저 `sessionStorage` 에만** 산다 — 서버로는 요청 헤더로만 가고 Core 는 저장하지
> 않는다. `localStorage` 가 아닌 것은 의도다: **탭을 닫으면 사라진다.** 공용 PC 에서 화면만
>보고 일어서도 키가 남지 않는다. URL 에는 절대 싣지 않는다(쿼리스트링은 로그·기록·Referer 로 샌다).
>
> **`/ui/redeem.html` 만 예외다 — 관리 키를 아예 만지지 않는다.** 초대받은 사람에게는 키가
> 없고 **초대 토큰 자체가 인증**이기 때문이다. 그 화면에는 등급·조직·티어 입력칸도 없다 —
> 초대장이 정하고 요청은 주장하지 못한다 (절대규칙 4). 이 둘은 `test_ui_invariants` 가 지킨다.
>
> 터미널로 운영하려면 `scripts/` + `ccurl`(키 주입) —
> [`operate-production.md`](./operate-production.md).
