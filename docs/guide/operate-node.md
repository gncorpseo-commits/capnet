...
> **강제 모드에서 이 UI 는 쓰지 못한다 — 데모·개발 전용이다.**
>
> UI 는 관리 API 키를 **보내지 않는다** (키 입력칸이 없다). 그래서
> `REQUIRE_API_KEY=1`(제품 프로파일)에서는 등록·발급 같은 **쓰기가 401** 이고,
> 조회면에도 역할이 걸려 있으므로(read-auth) **함대·작업 조회도 401** 이다.
>
> 쓰기가 401 인 것은 **전부터 그랬다.** 읽기까지 막힌 것이 read-auth 로 달라진 점이고,
> 「쓰기만 잠그면 증적이 누구에게나 보인다」를 닫기 위해 그렇게 했다.
> **키 입력칸을 붙이는 것은 별건**으로 남겨 뒀다 (Decision: 이번 범위 밖).
>
> 제품 프로파일에서는 `scripts/` + `ccurl`(키 주입)로 운영한다 —
