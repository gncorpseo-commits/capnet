...
- **조회면에도 역할이 걸린다** (read-auth). 함대·작업·증서 조회는 `developer`/`admin` 키가 필요하고,
  `GET /v1/tasks/{id}` 는 **자기 작업만** 보인다(남의 것은 404). 그래서 **최소 UI(`/ui/*.html`)는
  강제 모드에서 쓰지 못한다** — 키를 보내지 않기 때문이다. 데모·개발 전용으로 본다.
  운영은 `scripts/` + `ccurl`(키 주입)로 한다.
- **조직(tenant) 경계는 아직 없다.** `app_user` 에 소속이 없어서 `developer` 이상 키 하나면
  **다른 tenant 의 작업도 보인다.** 지금의 tenant 격리는 「어디서 실행되나」(라우팅)까지다.
  다중 조직에 열려면 이 칸이 먼저다 — 별 Proposal.
