...
> **화면은 검사가 실행해 본다.** `capreq/tests/chat_render_probe.js` 가 `chat.html` 의
> `<script>` 를 최소 DOM 스텁으로 돌려 **만들어진 DOM 을 단언한다** (`node` 필요 ·
> **npm 패키지 0** · 없으면 skip). 실제 브라우저의 CSS·상호작용은 **여전히 못 본다.**
