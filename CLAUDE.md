...
7. **입력은 통제된 수집만 받는다 (D8′).** 금지 대상은 「자유 업로드」가 아니라 **비통제 수집**이다 — 서명 URL·`fileToken`처럼 Core가 검증과 증적을 갖지 못하는 형태는 계속 기각한다. 허용은 **Core 중개**뿐: 입력 계약(`capability.input_schema`)·해시 증적·크기·MIME·보존 정책 아래의 수집. allowlist된 `datasetId`는 폐기하지 않고 **데모·카탈로그 보조 경로**로 남는다. **자유 업로드 경로를 만들지 않는다.**
...
- **측정 숫자는 재현 명령 없이 쓰지 않는다.** 카탈로그·`STATE.md`에 `acc=`·홀드아웃 `N/M`·ms 같은 **재야 나오는 값**을 적을 때는 같은 커밋에 그것을 다시 낼 명령이나 `scripts/` 도구를 붙인다. 없으면 숫자를 쓰지 않는다. 「9종」처럼 **세면 나오는 개수**는 대상이 아니다. 정본 → `@docs/guide/measured-claims.md`
...
- **`main` 머지는 master/사람이 한다.** PR까지 올리고 멈춘다.
  **예외 (11회차 · 2026-09-06 사용자 승인 · 배치 C–D·최종 G · **배치 R 연장**):** Claude 가 **자기 스택**을
  머지한다. 조건 전부를 만족할 때만 — CI **3/3 전부 pass** · 변경이 `tests/`·`docs/`·`scripts/`
  안 · 뮤테이션 ≥2 를 실제로 돌려 PR 에 적음. 체크가 도는 중 머지 금지(#290).
  **다음은 그대로 사람 몫이다:**
  ...
  **시드 종료 뒤는 배치 R — Cursor 재전달을 기다리지 않는다.**
