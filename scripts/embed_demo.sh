#!/usr/bin/env bash
# text.embed 종단 데모 (단계 6 ①) — structured 출력이 실제로 검증되는가
#
...
#
# 무엇을 주장하지 않나
#   분류 성능. `text.embed` 는 `quality_profile='none'` 이라 골든셋도 채점도 없다.
...
weights="text_embed_scratch.safetensors"
...
code=$(ccurl -s -o /dev/null -w '%{http_code}' -X POST "$core/v1/arches" \
  ...
  -d "{\"arch\":\"$arch\",\"max_params\":500000,\"note\":\"text.embed 참조 사영 (단계 6) · 4096x64 = 262,144\"}")
echo "  HTTP $code"
...
cap_body='{
 "code":"text.embed","version":1,"name":"text embed (fixed projection)",
 "description":"64차원 벡터 · 고정 시드 사영 · 의미적 유사도 주장 없음",
 ...
 "output_schema":{"required":["vector"],"properties":{
   "vector":{"type":"array","items":{"type":"number"},"minItems":64,"maxItems":64}},
...
print(next(c["id"] for c in items if c["code"]=="text.embed" and c["version"]==1))')
  echo "  이미 있음 → $capid"
...
inp=$(ccurl -sf -X POST "$core/v1/inputs?capability=text.embed&version=1" \
...
agent=$(ccurl -sf -X POST "$core/v1/agents" -H 'content-type: application/json' -d "{
 \"name\":\"text-embed-scratch\",\"version\":\"$ver\",\"manifest_hash\":\"text-embed\",
...
CORE_URL="$core" bash "$root/scripts/contract_bind.sh" \
  --agent "$agentId" --capability text.embed@1 --weights "$weights"

echo "== 6) 능력만 요구한다 — 기기 주소 없음 =="
task_in="$(mktemp -t capnet-task-XXXXXX.txt)"
printf 'https://capnet.example.org/docs/spec' > "$task_in"
tin=$(ccurl -sf -X POST "$core/v1/inputs?capability=text.embed&version=1" \
...
task=$(ccurl -sf -X POST "$core/v1/tasks" -H 'content-type: application/json' \
  -d "{\"datasetId\":\"text-demo\",\"caseId\":\"url-1\",\"capability_code\":\"text.embed\",\"capability_version\":1,\"inputId\":\"$tid\"}")
...
v = res.get("vector") or []
if len(v) != 64:
    raise SystemExit("벡터가 64차원이 아니다: %d" % len(v))
if "label" in res:
    raise SystemExit("임베딩 결과에 label 이 들어 있다 — 증적이 거짓말한다")
print("embed demo OK — structured 출력이 계약 게이트와 실행 경로를 완주했다")
...
rm -f "$sample" "$task_in"
echo
echo "의미적 유사도를 주장하지 않는다 — 고정 시드 사영이고 quality_profile='none' 이다."
