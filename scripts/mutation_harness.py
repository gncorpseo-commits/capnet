...
MUTATIONS: list[dict[str, str | None]] = [
    ...
    {"id": "compose-guard-not-called", "file": "scripts/lib/http.sh",
     "find": "\n_capnet_assert_compose_matches_core\n", "replace": "\n",
     "test": "tests.test_compose_project_follows_core_url", "why": "가드를 정의만 하고 안 부른다 (R3)"},
    {"id": "comment-only-compose-guard", "file": "scripts/lib/http.sh",
     "find": "\n_capnet_assert_compose_matches_core\n", "replace": "\n# _capnet_assert_compose_matches_core\n",
     "test": "tests.test_compose_project_follows_core_url", "why": "주석으로 옮겨 가드를 끈다 (R3)"},
    {"id": "compose-guard-always-agrees", "file": "scripts/lib/http.sh",
     "find": '  [ "$published" = "$want" ] && return 0\n', "replace": "  return 0\n",
     "test": "tests.test_compose_project_follows_core_url", "why": "포트를 비교하지 않고 늘 통과 (R3)"},
